...
| [[../10-Architecture/09-Decisions/ADR-0058-community-overlay-pipeline-context]] | proposed | Proposed Community Overlay Pipeline context owns the pack registry (platform-scope catalog), import-session FSM (uploaded → validating → approved → activated → archived), manifest schema validation, conflict-resolution policy, IP-safety gate + audit log, per-save active-packs snapshot (immutable at save creation); orchestrates semantic-validation delegations to Regulations + Rivalry + Tactics + Squad + Club per Vernon Process Manager / Saga pattern; canonical ingestion-as-bounded-context (Stripe Connect / Avalara / Salesforce / GitHub Actions / OpenStreetMap analogues). | New FMX-33 draft. Synthesis [[../60-Research/community-overlay-pipeline-bounded-context-2026-05-28]] recommends Option D (own context). Six-of-six DDD split criteria fire. **Existing FMX delegation pattern**: ADR-0056 + ADR-0057 explicitly reference "FMX-33 Community Overlay Pipeline" as upstream orchestrator. `risk:legal` label set. Awaiting Nico ratify decision. |
