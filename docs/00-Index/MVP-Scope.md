...
| Core systems | Weekly heartbeat, fictional club creation, basic squad, tactics, first match loop, Club Economy MVP pillar, AI Narration MVP pillar, manager-archetype run-analysis hooks, feed-card guidance. |
...
| Free-form AI chat or LLM-authored gameplay decisions | Out of MVP. MVP dialogue is intent-based and generated prose is presentation only. |
...
| AI narration/dialogue | Template fallback is always available. Runtime-LLM enhancement requires server/provider availability and never finalises authoritative effects. | Future local or alternate provider adapter may enhance copy, but generated prose stays non-authoritative. |
