...
> - „iOS-Safari 7-Day-Eviction wiped installed saves on non-home-screen users — 10 % first-week saves silently weg auf 30 % iOS-DAU."
> - „QuotaExceededError mid-save on Floor-Android with Large-world save."
> - „Brotli-CompressionStream Annahme bricht Chrome ≤ 138 + Firefox ≤ 146 — alle Save-Writes fehlschlagen."
> - „Match-Worker OOM-killed auf 3 GB Android während 90-min Live-Match — 45 Minuten Play verloren."
> - „`navigator.deviceMemory` undefined auf iOS Safari + Firefox → 50 % User mis-tiered, Premium-iPhones in Floor-Mode."
...
**Mitigation.** (1) **Pre-write quota probe**: vor jedem Save `estimate()`; `free = quota - usage` vergleicht mit `serializedSize × 1.2`. Bei < Marge → Soft-Refuse mit UI „Speicher fast voll, lösche alte Replays". (2) **LRU-Eviction-Tier in `save_blobs`**: Replays > 30 in-game years getaggt `cold_lru_eligible=true`. Bei `usage > 0.7 × quota` → automatic eviction bis < 0.5. (3) **World-Size-Refuse on Floor**: Floor-Tier kann Medium/Large öffnen aber NICHT erstellen; UI „Dein Gerät ist für Small empfohlen". (4) **Atomic-Transaction-Recovery**: IndexedDB-Transactions atomar (Dexie 4); bei `QuotaExceededError` rollt zurück, vorheriger Save überlebt. UI „Speicher voll" + Export-Anstoß. (5) **Storage-Buckets-API (post-MVP)**: Chrome 122+ stable; `career-buckets/main` trennt Save-Storage vom Asset-Cache. Firefox/Safari noch nicht; feature-detect + Fallback.
...
**Verifikation.** Chrome DevTools Memory-Pressure-Sim in Playwright. LambdaTest Phase 2 auf Galaxy A12 (3 GB) mit anderen Apps offen, 90-min-Match → assert match_resumes_from_last_flush.
...
5. UX: In-App „Storage full" Deep-Link zu Export+Delete-Management.
...
3. Bei > 0.5 %: In-App-Banner global „iOS users: install to home screen".
4. A8HS-Onboarding-Redesign-Sprint re-priorisieren.
5. Email-Comms an Affected (falls Email captured): „we've lost data, here's how to prevent it".
