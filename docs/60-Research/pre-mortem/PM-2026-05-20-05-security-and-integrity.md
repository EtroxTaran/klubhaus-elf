...
> - „Ein Tampering-Skandal zerstörte das Vertrauen in async Liga-Resultate — 60 % der Premium-Spieler kündigten."
> - „Ein Save-Forgery-Tool kursierte im Discord — Hall of Fame war Wochen lang unbenutzbar."
> - „Ein npm-Supply-Chain-Angriff schleuste Code in die Match-Engine ein — wir mussten wochenlang offline gehen."
> - „Eine DSGVO-Anfrage zu einem gelöschten Account ergab, dass PII in Loki-Logs steckte — Bußgeld + Reputationsschaden."
...
**Hypothese.** Aktuell vertrauen Server-Routen der Session und akzeptieren JSON-Bodies. Ein authentifizierter Spieler kann via DevTools beliebige Commands ausführen (z. B. „Vertrag mit -1 € Wage abschließen"); ein passiver Beobachter kann Commands replay'en, wenn Session-Cookies leaken.
...
**Mitigation.**
1. CI-Job „Determinism Gate": 1.000 fixe Seeds × Replay × Bit-Equal-Assert.
2. Stryker-Mutation darf den Determinism-Test nicht überleben (separate non-mutated „integrity"-Test).
...
**Hypothese.** Webhook-Endpoints (Stripe-IPN, GitHub-MCP für Auto-PR-Reviewer, sentry-glitchtip-Alerts) sind öffentlich erreichbar. Ohne Signatur-Validation kann ein Angreifer „bezahlte" Entitlements grant'en oder Audit-Log-Spam verursachen.
...
**Hypothese.** Forensik nach Vorfall („wer hat die Lineup verändert?" / „warum hat das Match dieses Resultat?") braucht ein durchsuchbares, append-only Audit-Log. Wenn das fehlt oder nur via Outbox-Lag verfügbar ist, sind Incidents oft nicht beweisbar.
...
4. **Comms:** Status-Page, Discord-Ankündigung; klare Trennung „weiterspielen ja / Leaderboards pausiert ja".
...
**Kosten dieser Disziplin: gering.** ~1–2 ms zusätzliche CPU pro SP-Match-Tick, +30 % Storage pro Save (Merkle-Tree-Footprint). **Nutzen: groß.** Wir bauen keinen zweiten Stack, MP/BYOC ist nicht „bolt-on" sondern „switch-on".
