...
> - „Tactic-Board-Drag verletzt WCAG 2.5.7 — kein Single-Pointer-/Tastatur-Pfad, Halbzeit-Wechsel für motorisch eingeschränkte Spieler unbenutzbar."
> - „BFSG-Geltungsbereich falsch eingeschätzt: B2C-E-Commerce-Funktion ab 28.06.2025 — Bußgeld bis € 100 k + Marktrücknahme."
> - „Live-Region des Match-Tickers `aria-live=assertive` — Screenreader-User können nichts anderes mehr fokussieren."
> - „shadcn/ui Default-Slate auf weiß: Kontrast nur knapp 4.5:1; Status-Badges reine Farbsignale — 8 % männliche User mit Farbsehschwäche ohne Signal."
> - „Squad-Tabelle als `<div>`-Grid statt semantisches `role=grid` — JAWS Browse-Mode kollabiert, 1500-Spieler-DB nicht navigierbar."
...
**Mitigation.** **react-aria** `useDraggableCollection` / `useDroppableCollection` — out-of-the-box Tastatur-Modus (Enter aufnehmen, Tab zwischen Drop-Targets, Enter ablegen, Esc abbrechen) + SR-Announcements. Zusätzlich „Verschieben nach…"-Button pro Spielerkarte → Combobox-Menü (Single-Pointer-Pfad). Halbzeit-Modal: zwei Listen „Aktuell auf dem Platz" / „Bank" + Checkboxen + „Tausch ausführen".
...
**Hypothese.** Single-Founder denkt „ist nur ein Spiel, keine Bank". Tatsächlich macht jede E-Commerce-Komponente die App BFSG-pflichtig. Verstoß = Bußgeld bis € 100 k, Marktrückruf durch Landesmarktüberwachung, Verbandsklagen.
...
**Mitigation.** Global CSS: `@media (prefers-reduced-motion: reduce) { *, *::before, *::after { animation-duration: 0.01ms !important; transition-duration: 0.01ms !important; scroll-behavior: auto !important; } }`. Match-Sim: statt Pan → diskrete Frames mit Highlight-Marker. In-App-Toggle „Animationen reduzieren".
...
**Mitigation.** `<Dialog.Root>`/`<Dialog.Content>` mit `<Dialog.Title>` (sichtbar) + `<Dialog.Description>`. Sub-Liste als Listbox mit `aria-activedescendant` ODER zwei Buttons „Auswechseln" / „Verwerfen". Trigger-Button erhält Focus zurück.

**Verifikation.** Storybook Story „Halftime Modal Keyboard-Only" + axe `aria-dialog-name`.
...
- **Component**: jede `*.stories.tsx` enthält „keyboard-only" Story-Variant.
