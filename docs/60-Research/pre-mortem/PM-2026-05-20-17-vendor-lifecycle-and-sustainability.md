...
> **Failure-Headlines**
> - ”žHetzner suspendet Account am Launch-Wochenende — 10 k Spieler offline 72 h — kein zweiter Cloud-Account."
> - ”žSurrealDB 3.x breaking-change v3.4 Computed Fields — Backup-Restore-Drill schlug nie an — 14 PT manuelle Recovery."
> - ”žStripe friert Auszahlungen ein — 'Football Manager' als Gambling-adjacent geflagged — 90-Tage-Reserve auf €4.500 — kein Paddle-Backup."
> - ”žTanStack Start stagniert ab Q4/2026 — Tanner Linsley auf Router-only — Migration zu Next.js 16 = 30–40 PT."
> - ”žCRA-Stichtag 11.12.2027: SBOM-Pflicht nicht eingehalten — Marktverbot EU + €15 M Bußgeldrahmen."
...
**Verifikation.** SLO ”žDB-Restore < 30 min für 10k-DAU-Snapshot" als CI-Gate. SurrealDB-Release-Notes monatlich diffen.
...
linked_adrs: []
linked_specs: [[ADR-0017-observability-logging]]
linked_code: []
sources:
...
sources:
  ...
  - title: "Nowtricity Germany"
    ...
    confidence: medium
verification_notes: "Hetzner DE 100 % Hydropower (Eigenangabe); PUE 1.13 best-in-class. HT Clean Energy GmbH 2024 für Solar-Parks. 2025-CSRD-pflichtig. FI-Nordischer Mix ~50–70 gCOâ‚‚eq/kWh vs DE-Grid 328 gCOâ‚‚eq/kWh. Football-PWA + Match-Sim CPU-light → Footprint < 5 kg COâ‚‚eq/Monat MVP."
resolved_by:
  - [[../../95-Archive/gap-reports/gap-closure-concept-2026-05-22]]
status: mitigated
owner_suggested: founder
...
**Mitigation.** (1) **Marketing-Claim** ”žHosted on 100% renewable energy (Hetzner DE)" zulässig — referenz auf Hetzner-Dok. (2) **NICHT** ”žCarbon-Neutral" sagen — Hetzner explizit nicht klimaneutral. (3) **Match-Sim auf FI-Region** wenn Latenz erlaubt — niedrigster COâ‚‚eq/kWh. (4) **SCI-Score** per Functional Unit (per Match-Sim) berechnen mit https://github.com/Green-Software-Foundation/sci (ISO 21031:2024).
...
3. Total estimated kgCOâ‚‚eq/Monat for production stack — `docs/40-Compliance/carbon-budget.md`.
4. Vendor-List with EU-residency + License-Note.
5. Voluntary VSME-Statement aktualisiert vor 31. März jeden Jahres.
6. Keine ”žcarbon-neutral"-Claims ohne Verifikation (EU Green Claims Directive in Kraft 2026).
...
**Carbon-Footprint (geschätzt, kgCOâ‚‚eq/Monat):**

| Scenario | Calc | kgCOâ‚‚eq |
|---|---|---|
| MVP single CX22 (Hetzner DE 100 % hydro) | ~20 W × 730 h × 10 gCOâ‚‚eq/kWh | **~0.15 kg** |
| 10k DAU prod stack (3× CX42 + LB + DB) | ~250 W × 730 h × 10 gCOâ‚‚eq/kWh | **~1.8 kg** |
| Hypothesis DE-Grid-Mix (328 gCOâ‚‚eq/kWh) | × 32.8 | ~60 kg |
| Client 10k DAU × 5 min/d × 2 W mobile | 10k × 5/60 × 2 × 30 × 250 g | ~25 kg |
| **Total server+client (10k DAU/Mo)** | | **~27 kg COâ‚‚eq** |

Vergleich: ~27 kg/Mo = ~100 km Auto (Benzin) pro Monat für 10k aktive Spieler. **Server-Side < 2 kg** → ”žGreen Indie"-Claim defensiv haltbar.
...
3. **Public Comms**: `status.<domain>.eu` (hosted bunny.net Static Edge, AUSSERHALB Hetzner). Twitter/Mastodon ”žInfrastructure issues, ETA <X>."
...
1. Convene ”žframework review" (1-day spike).
...
### RB-17-C: Stripe-Account-Closure (”žhigh-risk reason")
...
4. **Comms an Active-Subscribers** (~100 wenn early): Brevo-Email ”žWe're moving payment processors; subscription paused; cancel anytime."
