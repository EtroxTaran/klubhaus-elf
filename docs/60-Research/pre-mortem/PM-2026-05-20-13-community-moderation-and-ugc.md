...
# Pre-Mortem 2026-05-20 · 13 · Community, Moderation & UGC

> **Failure-Headline-Kandidaten**
> - „3 Custom-Wappen mit Hakenkreuz schlüpften durch Upload-Filter — Reddit-Thread #FMXnazi 14 k Upvotes — Strafanzeige § 86a StGB gegen Founder als Hoster."
> - „DFL-Anwaltsschreiben wegen 4.218 Save-Files mit 'FC Bayern München' inkl. Wappen — 14-Tage-Frist."
> - „DSA Art. 16 Notice 11 Tage ignoriert (Founder im Sommerurlaub) — DSC-NRW-Beschwerde — Bußgeld-Androhung bis 6 % Umsatz."
> - „Discord-Raid 200 Bot-Accounts in 90 Sek mit rassistischen Tactic-Namen; Auto-Mod schlug 12.000× Alarm; Founder kapitulierte 03:47 Uhr."

## Top Failure-Hypothesen

### PM-2026-05-20-13-F-01 — DSA Art. 16 Notice-and-Action ohne SLA-getragene Prozessmaschine
...
early_warning:
  ...
  - metric: "dsa_notice_response_time_p95_h"
    threshold: "> 72 h für klare Fälle"
...
sources:
  ...
  - title: "Notice and Action Requirements (De Gruyter)"
    ...
    confidence: medium
verification_notes: "Welle-1-Report-08 hat Pflicht nominal anerkannt, aber kein vollständiges Notice-Formular spezifiziert. Penalty bis 6 % weltweiter Jahresumsatz (Art. 52). SME-Ausnahme Art. 19 entlastet von Risk-Assessment, NICHT von Art. 16."
status: open
...
**Mitigation.** (1) Standalone Page `/legal/notice` mit Form-Felder + JSON-Schema-Backend. (2) Mail-Fallback `notice@…` (Art. 16 (1) S. 2 elektronische Übermittlung). (3) Ticket-System (Plane/Linear API oder GitHub-Issues mit Label `dsa-notice`). (4) SLA: `acknowledgement < 24 h`, `decision < 7 d` für klare Hassrede/§-86a-Fälle, `< 14 d` IP-Trademark. (5) Bona-fide-Erklärung als Checkbox mit Legal-Text. (6) Automatisierte SoR an Notifier + Uploader.

**Verifikation.** Synthetic Notice-Submission jede Woche durch internen Bot; SLA-Burn-Rate-Alert bei p95 > 50 % SLA-Budget.

### PM-2026-05-20-13-F-02 — § 86a StGB-Risiko bei Custom-Wappen-Upload (strafrechtlich, nicht zivil)
...
early_warning:
  ...
  - metric: "image_upload_symbol_classifier_hit_total"
    threshold: "> 5/week"
mitigation_summary: "MVP: Wappen-Upload DEAKTIVIERT — prozedurale Wappen aus Klubname-Hash + Farbpalette + 200 SVG-Templates. Wave 2 nur via pHash + Symbol-Classifier + Human-Review-Hold"
...
sources:
  - title: "§ 86a StGB"
    ...
    confidence: high
  - title: "BzKJ § 86a StGB Anbietersicht"
    url: "https://www.bzkj.de/resource/blob/130176/529e31da65b8e3e4a01399f038736771/201802-86a-aus-anbietersicht-data.pdf"
    accessed: "2026-05-20"
    publisher: "BzKJ"
    confidence: high
verification_notes: "§ 86a StGB Freiheitsstrafe bis 3 Jahre. Plattform-Privileg (§§ 7–10 DDG) greift erst ab positiver Kenntnis. Bei viralem Hakenkreuz-Wappen kann StA gegen Founder ermitteln. Custom-SVG ist XSS-Vektor."
status: open
...
**Mitigation.** (1) **MVP-Beschluss: Kein Custom-Wappen-Upload.** Prozedural generierte Wappen aus Klubname-Hash + Farbpalette + 200 vorbereiteten SVG-Templates. (2) Wave 2 mit Upload: Format-Whitelist `image/png|image/webp|image/jpeg`, **niemals SVG**. Sharp-Pipeline rasterisiert + strippt Metadata + Density-Cap. 256×256 px, 256 KB Max. (3) Pipeline: Upload → Sharp re-encode → pHash + dHash → DB-Lookup gegen Symbol-Hash-List (BPjM-Indizierungsliste + community-pflegt). Match → Reject + Soft-Flag. (4) Symbol-Classifier (Llama-Guard-Vision oder Hive Visual Mod „Hate-Symbols") als zweite Stufe. (5) Human-Review-Hold 60 Min, ID-Card-Style „pending review" bis Founder/Mod approved (Discord-Embed-Slash-Action). (6) Rate-Anomalie (> 3 Uploads/h/Account) → Auto-Suspend + Push.

**Verifikation.** 30 Symbol-Varianten (Rotation, Spiegelung, Pixel-Noise) gegen Upload → alle reject/hold. JS-in-SVG: MIME-Check + Magic-Bytes-Prüfung blockt.

### PM-2026-05-20-13-F-03 — Vereins-Trademark-Blocklist DFL/UEFA = Build-Time-Daten + Runtime-Match-Pipeline
...
early_warning:
  - metric: "club_name_blocklist_hit_total"
    threshold: "Spike > 100/day → False-Positive-Review"
  - metric: "trademark_complaint_open_total"
    threshold: "> 0"
mitigation_summary: "Wikidata-getriebene Blocklist (BL + 2.BL + UEFA-Top-100) mit jährlichem Cron-Build, Levenshtein ≤ 2 + Unicode-NFKC + Leet-Decoder + Phonetic-Fallback"
linked_adrs: []
...
sources:
  - title: "Bundesliga legal notices"
    ...
    confidence: high
verification_notes: "Naive Exact-Match scheitert: User tippen `FCBayern2026`, `Bayern_Munich_88`, `FC Вayern` (kyrillisches В), `B@yern Munchen`. Ohne Fuzzy+Confusable+Token = nutzlos."
status: open
...
**Mitigation.** Vollständiges 6-Stufen-Matcher-Schema (siehe Block-List-Implementation unten): NFKC + Confusable-Decode → Leet-Decode → Token-Strip + Bag-of-Tokens → Exact-Match → Fuzzy (Jaro-Winkler ≥ 0.92) → Phonetic (Double-Metaphone).

**Verifikation.** Bypass-Test-Suite: 200 generierte Varianten von 20 Schutz-Namen; ≥ 95 % blockiert; FPR auf 100 legitimen ≤ 2 %.

### PM-2026-05-20-13-F-04 — Single-Founder als Mod = Burnout + SLA-Verletzung
...
early_warning:
  - metric: "mod_queue_depth"
    threshold: "> 50 outstanding für > 24 h"
...
sources:
  - title: "On-Call Scheduling for Small Teams (Hyperping)"
    ...
    publisher: "Hyperping"
    confidence: medium
verification_notes: "Founder ohne Mod-Team kann 7-Tage-DSA-SLA nur halten bei (a) ≤ 5 Notices/Tag und (b) keinen Urlaub. Bei 10k DAU mit 5–15 Notices/Tag realistisch."
status: open
...
**Mitigation.** (1) **Tier-0 (Filter)**: AI-Pre-Filter (Llama-Guard 3 self-hosted EU + Profanity-Regex + Blocklist) blockiert vor Veröffentlichung. (2) **Tier-1 (Trusted Community-Mod)**: 3–5 Free-Tier-Spieler mit `community-mod`-Discord-Rolle. Rechtsstatus: keine Vertretungsmacht ggü Plattform-Pflichten; nur vorläufig verbergen; Final-Decision Founder. Mod-Charta + DSA-Haftungsausschluss. (3) **Tier-2 (Founder)**: Final-Decision + Statement of Reasons. (4) **Eskalations-Cache**: 72-h-Urlaub-Modus, AI-Filter aggressiver, alle Uploads `pending`.

**Verifikation.** Wöchentlicher SLA-Burndown; bei `mod_queue_depth > 50` oder p95 > 48 h → Discord-Webhook + SMS.

### PM-2026-05-20-13-F-05 — Hate-Speech-Bypass durch Leet/Unicode/Padding
...
sources:
  ...
  - title: "Homoglyph Attack 2025"
    ...
    confidence: medium
verification_notes: "Spieler umgehen Wordlist trivial: Fußba11, Sсhei$$e (kyrill. с), f u c k (Padding), fuuuuck (Repetition), 🅵🆄🅲🅺 (Emoji-Stems)."
status: open
...
**Mitigation.** 3-Stufen-Pipeline: (1) Unicode-NFKC + confusable-homoglyphs → ASCII. (2) Obscenity / GLIN-Profanity (Transformer-basiert, matched `fuuuuck` + `wordsbeforefuckandafter`, `disableLeetSpeak: false`). (3) Perspective API oder lokale **Llama-Guard-3-8B** EU-Hosted (F1 0.939). Score > 0.85 → Reject + log + repeat-offender++. Score 0.6–0.85 → soft-flag.

**Verifikation.** Bypass-Test-Suite 500 Varianten Top-30-Schimpfwörter: ≥ 90 % Recall, FPR auf 200 legitimen ≤ 3 %.

### PM-2026-05-20-13-F-06 — Repeat-Offender-Policy (Art. 23 DSA) fehlt
...
early_warning:
  - metric: "user_strike_count_p99"
    threshold: "p99 > 2 → Policy-Bedarf bestätigt"
mitigation_summary: "3-Strike + Cooldown-Eskalation (1h → 24h → 7d → term), Prior-Warning vor Suspension, in T&C + DSA-Transparency-Report"
linked_adrs: []
linked_specs: []
...
sources:
  - title: "DSA Art. 23 Repeat Infringers"
    ...
    publisher: "eu-digital-services-act.com"
    confidence: high
verification_notes: "Art. 23 (1) DSA: 'häufig offensichtlich illegale Inhalte' → Suspension. Ohne dokumentierte Policy juristisch angreifbar (Art. 54 Schadenersatz)."
status: open
owner_suggested: legal+platform
effort: S
created: 2026-05-20
...
**Mitigation.** Strike-Schema: Strike 1 (Hate-Speech-Klubname) → Warning + reset auf default. Strike 2 → 24 h Read-only. Strike 3 → 7 d UGC-Sperre. Strike 4 → Account-Termination + 6-Mo-Daten-Retention (forensisch, kollidiert mit Art. 17 → F-09). Pre-Warning bei jedem Strike (Art. 23 (2)). Transparency-Report Art. 24 (3): Anzahl Suspensions je Kategorie. **Hinweis**: SME-Ausnahme gilt nicht für Art. 24 (3), nur (1)+(2).

### PM-2026-05-20-13-F-07 — Translation-UGC (post-MVP TMS) als unmoderiertes Hate-Vehikel
...
sources:
  - title: "Tolgee GitHub Platform"
    ...
    publisher: "Tolgee"
    confidence: high
verification_notes: "Italienische Übersetzung mit faschistischen Slogans in obskuren Tooltips. Ohne 2-of-N-Reviewer geht das in Production."
status: open
owner_suggested: founder
effort: M
created: 2026-05-20
...
**Mitigation.** (1) TMS als GitHub-PR (Crowdin/Tolgee → GitHub-Sync) — Founder merged explizit. (2) Pro Sprache 2 Native-Reviewer mit `translator`-Rolle. (3) Vergütung: Discord-Rolle + Credits (kein Geld → keine Arbeitnehmer-Konstruktion → DSGVO-Auftragsverarbeitung nicht nötig). (4) Diff-View, kein Bulk-Approve. (5) Post-Merge AI-Scan auf Toxicity-Drift.

**Verifikation.** CI-Gate: Locale-File-Diff nicht ohne Approver-Signatur in `main`.

### PM-2026-05-20-13-F-08 — Async-Multiplayer-Chat als unkontrolliertes UGC-Forum (Post-MVP)
...
sources:
  - title: "DSA Article 16 (private content debate)"
    ...
    publisher: "CMS"
    confidence: medium
verification_notes: "Art. 16 strittig für private Nachrichten, aber sicherheitshalber Notice-Mechanismus unabhängig von Öffentlichkeit."
status: open
owner_suggested: founder
effort: L
created: 2026-05-20
...
**Verifikation.** Spec-Review: MVP-Roadmap enthält keinen Free-Form-Chat. Wave-2-ADR muss Filter spezifizieren vor Live.

### PM-2026-05-20-13-F-09 — DSGVO-Konflikt: Notice-Reporter-PII vs Aufbewahrung für Audit
...
early_warning:
  - metric: "gdpr_erasure_request_overlap_with_notice_records_total"
    threshold: "> 0"
mitigation_summary: "Pseudonymisierte Notice-Records (Notifier-Hash + verschlüsselter Mail-Vault); 6 Mo. Aufbewahrung; Lösch-Antrag → Pseudonymisierung statt Komplett-Löschung"
linked_adrs: []
...
**Mitigation.** Notice-Record-Schema: `notifier_email_encrypted` (separater KMS-Key), `notifier_email_hash` (SHA-256 für Audit). Löschantrag → KMS-Key-Rotation auf Hash-only, Plaintext irreversibel weg. Audit-Trail bleibt. Retention 6 Mo Default; T&C/Privacy-Policy explizit (Art. 14 Transparenz). Bona-fide-Erklärung enthält Verarbeitungs-Consent + Speicher-Info.
...
### PM-2026-05-20-13-F-10 — Username-Squatting auf Schutzmarken (FCBayern_2026, Real_Madrid_88)
...
sources:
  - title: "FIFPro license analysis (Soccerverse case)"
    ...
    confidence: medium
verification_notes: "User wählt `Lewandowski` als Manager-Bio → wir verbreiten Player-Likeness ohne Lizenz. FIFPro-Anwaltsschreiben droht."
status: open
owner_suggested: platform
effort: S
created: 2026-05-20
...
**Mitigation.** Username-Blocklist baut auf Club-Blocklist + Spieler-Top-3000 aus Wikidata Query (P54 member of sports team, top-leagues). Composite-Match: `BayernMunich88` zerlegt in Tokens [Bayern, Munich, 88]; Bayern+Munich kombiniert matched gegen `FC Bayern München`.

### PM-2026-05-20-13-F-11 — Save-Sharing-Forum (Post-MVP) = neuer UGC-Vektor
...
//    }
// 2. Wikidata "also known as" → aliases.
// 3. Normalize: NFKC, lowercase, strip diacritics, strip "FC|SC|TSV|…" prefix into aliases.
// 4. Double-metaphone für Phonetic-Fallback.
...
function checkClubName(userInput: string): MatchResult {
  ...
  normalized = confusableHomoglyphs.toAscii(normalized); // kyrill. о → o
  ...
  // Stage 5: Fuzzy (Jaro-Winkler ≥ 0.92 OR Levenshtein ≤ 2 für ≥ 5 chars)
  for (const entry of blocklistClubs) {
...
|---|---|---|
| Unicode-confusable | `FС Bayern` (kyrill. С) | 1 |
| Leet | `FC B4yern` | 2 |
...
|---|---|---|---|---|---|
| **Llama-Guard 3-8B self-hosted** | Text-Classifier | Hetzner GPU EU | Compute ~€120/Mo (A6000) | Voll DSGVO, audit-fähig | **MVP-Primary** Text |
...
| **OpenAI Moderation (omni-mod-latest)** | Text+Image | US | Free, kein DPA für non-DataPlus | EU-US-DPF erforderlich | Wave-2-Fallback non-persistent |
| **Hive Moderation** | Image/Text/Video | US | Custom $50–500/Mo SMB; opt-out 14d | DPA verfügbar | Image-Fallback wenn Llama-Guard-Vision unzureichend |
| **Sightengine** | Image/Text | **EU (FR) Option** | $29/$99/$399 | EU-Hosting verfügbar | **Image-Primary Wave-2** falls Wappen |
...
| **pHash + Symbol-DB** | Image-hash-match | local | Free | DSGVO | **MVP-Primary** §86a-Symbol-Hashes |
| Microsoft Content Moderator | — | — | EOL'd 2027 | — | nicht empfohlen |
...
**MVP-Stack:**
1. **Text-UGC**: Obscenity + confusable-homoglyphs (Stage 1) + Blocklist (Stage 4–6) + **Llama-Guard-3-8B** Hetzner GPU für Kontext-Toxicity.
2. **Image-UGC** (Wave 2 falls überhaupt): Sharp-Rasterize → pHash + dHash gegen lokale `extremism-symbol-hashes` → **Sightengine** EU für Nudity/Violence → Human-Review-Hold.
3. **Discord**: AutoMod + Wick (anti-raid + 14-d-account-age) + Carl-bot Roles. Max 3 Bots.

**Total laufende Kosten MVP**: ~€120 GPU + €0–29 Sightengine-Starter = **~€120–150/Monat**. Wave-2 mit Image-Upload realistisch €300–400/Mo.

## Quantitatives Modell

| Metric | MVP-Estimate (10k DAU) |
|---|---|
| Notices/Tag | 5–15 (0.05–0.15 % DAU) |
| Notices/Tag bei Viral | 200–1.000 |
| UGC-Felder/Spieler | ~6 (Klubname, Wappen, Manager, Bio, 3–5 Tactic-Names, Save-Title × N) |
| UGC-Submission-Rate | ~12/DAU/Mo |
| Pre-Filter Block-Rate | 0.5–2 % |
| Post-Publish Report-Rate | 0.05 % |
| Moderator-Stunden/10k DAU/Mo | 8–20 h |
| False-Positive Profanity | ≤ 3 % |
| False-Negative Hate-Speech | ≤ 10 % |
| Image-Upload bei aktiv | ~200/Tag |
| Symbol-Hit-Rate | 0.05–0.5 % der Uploads |
...
| `notice_decision_p95_h` | < 7 d klare Fälle / < 14 d komplex |
| `ugc_text_filter_false_positive_rate_pct` | < 3 % (wöchentliche Random-Stichprobe) |
| `ugc_text_filter_recall_hate_pct` | > 90 % (Bypass-Suite) |
| `blocklist_freshness_days_since_build` | < 365 d (Cron) + < 30 d ad-hoc |
| `image_upload_symbol_hash_db_lag_d` | < 14 d nach Symbol-Hash-Veröffentlichung |
...
**Bypass-Test-Suite**: 500 Varianten Top-30-Schimpfwörter + 200 Varianten Top-50-Klubnamen → ≥ 90 % Recall, FPR auf 200 legitimen ≤ 3 %.

## Runbook-Skizzen

### RB-13-A: DFL-Anwaltsschreiben re Vereinsname/Logo-UGC
1. **0–24 h**: Eingang `legal-incoming@`, Notice-ID, Empfangsbestätigung mit § 5 DDG-Anschrift.
2. **24–48 h**: DB-Suche aller Saves mit gematchtem Pattern. Bei Match: Soft-Hide 7 d + Notice-an-Uploader.
3. **48–72 h**: Schriftliche Antwort mit Hosting-Privileg-Hinweis (§ 7–10 DDG, DSA Art. 6) + Notice-and-Action-Bestätigung + Strike für Repeat-Uploader.
4. **7–14 d**: Blocklist-Erweiterung um spezifischen Trademark-Hit. Strike-Eskalation.
5. **Eskalation**: Bei Klage → External-Counsel (Härting Rechtsanwälte).

### RB-13-B: Massen-Hate-Speech-Spam-Welle
1. **T+0**: Anomaly-Detector bei > 20 UGC/Min mit Profanity-Score > 0.7.
2. **T+5 Min**: Auto-Throttle: UGC-Felder temporär 1 Submission/h, Pre-Filter-Threshold auf 0.5.
3. **T+15 Min**: Founder-SMS + Discord-Webhook.
4. **T+30 Min**: Bot-Origin → IP-Block Top-3 ASNs via Hetzner/Cloudflare. CAPTCHA-Gate temporär.
5. **T+1 d**: Backlog-Sweep: alle Submissions des Fensters manuell.
6. **T+7 d**: Post-Mortem + Counter-Tuning.

### RB-13-C: Schwerwiegender § 86a-Vorfall (Strafrecht)
1. **T+0**: Reporter-Notice / Tier-1 erkennt Hakenkreuz-Wappen.
2. **T+5 Min**: Sofort-Takedown (vor Diskussion). pHash + Original verschlüsselt in `evidence/`-Vault. **Nicht löschen.**
3. **T+30 Min**: Founder-Triage: Einzelfall oder Multi-Upload-Spree? Letzteres → RB-13-B.
4. **T+2 h**: Statement of Reasons an Uploader: Account-Suspension Strike 4. Daten 6 Mo Retention (forensisch).
5. **T+24 h**: Selbstanzeige bei StA *optional* — Anwalt-Konsultation zuerst. Hoster-Privileg schützt, aktive Anzeige umstritten (exponiert Reporter-PII).
6. **T+72 h**: Symbol-Hash zur lokalen `extremism-symbol-hashes`-DB. **Nie Originaldatei in DB.**
7. **Eskalation**: Bei Behörden-Anfrage → External-Counsel + Pseudonymisierung des Uploaders bis Beschlagnahme-Anordnung.

## Offene Fragen

1. **Founder-Privatadresse als Impressum?** § 5 DDG verlangt ladungsfähige Anschrift. Coworking oder Postfach-Service (Clevvermail Hamburg ~€30/Mo)?
...
3. **Selbst-Anzeige bei § 86a-Fund**: Pflicht oder Option? Konsens: keine Anzeige-Pflicht für Hoster, aber Beweismittelaufbewahrung muss strafprozessual standhalten.
4. **Wikidata-Source-of-Truth-Risiko**: Vandalism-Risk. Manuelle PR-Approval-Pflicht + Diff-Cap (> 5 % change = CI-Fail).
5. **Image-Upload überhaupt jemals?** Procedurale Wappen-Engine deckt 80 % Bedarf. Image-Upload öffnet § 86a-Box. Empfehlung: erst nach Series A.

## "Wenn wir nur 3 Dinge tun"-Liste

1. **MVP ohne Image-Upload + ohne Free-Form-Chat.** Procedural Wappen + Emote-Set decken ~80 % Bedarf. Eliminiert § 86a-Hauptrisiko + Single-Founder-Mod-Load-Risiko.
2. **Notice-and-Action-Pipeline (Art. 16 DSA) als hartes Launch-Gate.** Form, Mail-Inbox, Ticket-System, 24h-ACK/7d-Decision-SLA. Ohne das keine Public-Beta — Bußgeld-Risiko asymmetrisch.
