...
> **Failure-Headlines**
> - „Indie unter MIT — 8 Mo später dominieren 3 Hosted-Forks von Bootleg-Anbietern den DACH-Markt; Original 0 zahlende Kunden."
> - „Carry-Slot Roguelite generiert 47 BzKJ-Hotline-Beschwerden: Eltern berichten 12-jährige spielen nächtlich 4-Stunden-Insolvency-Runs."
> - „Welle-3-Audit Dark-Pattern-Drift: 'Letzte Chance' Banner + Auto-Opt-In + roter Exit/grüner Continue — DSA-Art-25-Beschwerde Verbraucherzentrale NRW."
> - „AGPL-3.0 gewählt zur Fork-Defense, aber Steam erfordert proprietären Lizenz-Stack — Steam-Launch +6 Mo License-Audit."
> - „Secret aus 2024: AWS-Access-Key in .env.example committed (vergessen rotiert), Repo public seit 2024 — Crypto-Miner findet Key Tag 3 — 4.700 € Cloud-Bill."
...
**Hypothese.** Insolvency-Spiral ist genre-typisches loss-aversion-design (legit) — wird Dark-Pattern, sobald (a) Carry-Slot-Auswahl unter Zeitdruck/Knappheit, (b) „Last Chance to Save Run"-Käufe, oder (c) versteckte/unklare „Was geht verloren?"-Kommunikation eingebaut.

**Mitigation.** (1) **End-of-Run-Screen** mit expliziter Liste: *Was bleibt (Carry-Slot, Hall-of-Fame, XP), was verschwindet (Squad, Liga, Vertrag)*. (2) Keine Zeitdruck-Choices auf Run-Ende-Screen (kein Countdown). (3) „Are you sure?"-Confirm bei voluntary Insolvency-Trigger. (4) Dark-Pattern-Self-Audit-Checklist (15 Items) pre-merge.
...
**Mitigation.** Opt-in Session-Clock + soft „Du spielst seit 90 min — Pause?"-Toast nach 90/120/180 min (~4 h Dev-Task). Settings → „Wellness" Toggle. Default ON für angemeldete <18 (wir haben 16+-gate, also 16/17-y-o); default OFF für 18+ mit prominentem Hinweis.
...
**Mitigation.** (1) Age-check VOR Telemetry/Account-Creation (bereits Plan). (2) Kein Telemetry-Persist bei „unter 16, exit" → ephemeral. (3) **IARC-Self-Rating (kostenfrei)**, Ergebnis auf Landing-Page. (4) Jugendschutzbeauftragter erst ab Schwellenwert (User-Decision: Trigger bei 50k MAU).
...
**Hypothese.** Wenn Game-State + Determinism-Seed-Engine + Liga-Backend server-side bleiben und NICHT in repo, ist Code-Fork ein nicht-funktionierender Client. „Open client, closed server"-Pattern (Element/Matrix-hub, Mastodon-instance). Permissive licenses werden damit weniger riskant.
...
linked_code: []
sources: []
verification_notes: "Discourse (radikal-offen): grosse Community kompensiert Roadmap-Diskussions-Overhead. Penpot: roadmap opaque, deliberate priorisation hidden = solo/small-team-pragma. GitHub-Issues + Milestones reichen für Solo."
status: open
owner_suggested: founder
effort: S
created: 2026-05-20
...
5. Spätere Relicense via DCO+contributor-replacement ODER „AGPL-3.0-or-later"-upgrade-clause.
...
|---|---|---|---|
| **Roguelite Insolvency-Spiral** (GD-0008) | loss-aversion / sunk-cost | **legitimate** (genre-norm, transparent, no monetization-coupling) | (a) End-of-Run-Screen explizit „what survives vs what's lost"; (b) no countdown timers on run-end; (c) "Are you sure?"-confirm on voluntary insolvency |
...
| **Carry-Slot Legacy** | meta-progression / reward-loop | **legitimate** if reveal-logic deterministic + transparent | (a) Carry-Slot pick UI zeigt ALLE eligible options (no hidden 'rare slot'); (b) no time-limited pick; (c) opt-out: „don't carry anything" 1-click |
| **Match-Sim Pre-Match Hype-Bar** (hypothetisch) | engagement-amplifier | **borderline** | Muss NOT auto-skip; user-controlled; no false-stakes („LAST CHANCE") wenn nicht faktisch |
...
| **Insolvency-Notification Push** (post-MVP) | guilt / nag-prompt | **dark-pattern-risk MEDIUM** | Frequency-Cap max 1/Woche pro Club; opt-in default OFF; „manage notifications"-Link in jedem Push |
...
| **Exit-Confirmation Dialog** | confirmshaming-risk | **borderline** | Wording neutral: „Exit game?" — NICHT „Sure? Your run will be lost forever! 😭". Buttons symmetric, no green/red-coding |
...
| **Push-Notification Re-Engagement 7d** | re-engagement-FOMO | **dark-pattern-risk MEDIUM** | Copy informational, nicht emotional („3 matches this week" nicht „Your team misses you!"); user opt-out per channel |
...
| **Carry-Slot „auto-pick best"-Suggestion** (hypothetisch) | nudge / default-choice | **borderline** | Falls implementiert: opt-in, nie default-pre-selected, gleiche Data-View für User und Algorithmus |
...
3. **Responsible-Gaming-Statement auf Landing-Page + Wellness-Settings mit opt-in 90-Min-Session-Reminder.** Null Monetization-Risk; setzt ethical Baseline; DSA-Art-25 „reasonable steps"-Defense-Evidence; differenziert von EA/2K/Konima im Football-Manager-Category durch Player-Care-Signal.
