...
> - „Wir haben Compute auf die Spielergeräte verteilt — eine Hand voll Cheater hat die Liga-Tabellen wochenlang gefälscht."
> - „Die Validator-Quorum-Logik war zu komplex — die Latenz war schlechter als bei Server-Berechnung, und wir mussten zurückrudern."
> - „Sybil-Angriff: ein Spieler hat 10 Accounts gleichzeitig als Validatoren registriert und Matches systematisch beeinflusst."
> - „Mobile Geräte konnten Match-Compute nicht zuverlässig liefern (Battery-Saver, Background-Tab-Kill), Latenz P99 ging auf 12 Min."
...
Match-Result = Quorum-of-(N=5) Re-Simulations mit Server-Fallback
              ├── Validator-Auswahl: deterministisch aus
              │     hash(worldSeed, matchId, week, validator_pool_state)
              ├── Inputs (Lineup, Tactic): server-committed BEFORE match-start
              │     (Commit-Reveal: hashes vor kick-off, reveal nach kick-off)
              ├── Engine-Bundle-Hash: per Validator gepinnt, Mismatch → reject
              ├── Validator-Output: signiertes Tupel
              │     (resultHash, eventLogMerkleRoot, validatorDeviceKey)
              ├── Quorum: 3-of-5 identische Result-Hashes → accept
              └── Disagreement: Server re-simuliert authoritativ → akzeptiert
                                Validator-Reputation: divergent → -3
                                                       consistent → +1
```

**Spannung zu ADR-0011.** ADR-0011 ist „Server-Authoritative" als MVP-Pattern. BYOC ist *„Quorum-Validated mit Server-Fallback"* — der Server bleibt finale Autorität (kann jederzeit re-simulieren), aber im Happy-Path spart er sich die CPU. Falls BYOC gebaut wird, beschreibt ein neuer `ADR-0027 BYOC Match Validation Quorum` die Erweiterung — ADR-0011 wird *nicht* superseded, sondern *ergänzt* mit einem Modus-Switch.
...
4. User-Toggle „Validator nur bei WLAN + Charger".
...
**Hypothese.** Reputations-System ist zu strikt (legitime Engine-Updates lassen alle ehrlichen Validatoren auf -∞ fallen) oder zu lax (Cheater bleiben aktiv).
...
4. „WLAN-only" und „Nur am Charger" als User-Settings.
...
- Community-Engagement („Du bist Teil der Liga-Infrastruktur") — Marketing-Wert schwer zu beziffern.
- Trust-Story („Ergebnisse von Liga-Mitgliedern verifiziert") — Glaubwürdigkeit.
...
| BYOC (Desktop, Browser offen) | ~3 s | ~30 s |
...
1. **Detect:** Reputation-Verteilung degenerate (z. B. alle -∞ nach Engine-Update).
...
3. **Baseline-Daten:** mindestens 1 Quartal Server-only-Betrieb in Production mit stabilem Determinism-CI-Gate, sodass „normal divergence rate" bekannt ist.
...
## „Wenn wir BYOC bauen, nur diese 3 Dinge"-Liste
...
Jedes Finding hat eine immutable ID (`PM-2026-05-20-06-F-NN`). Da BYOC Future-Scope ist, sind Findings hier **vorerst `open` und unter `accepted-risk` aggregiert** — sie werden aktiviert, sobald Decision-Gate erfüllt ist und Implementierung beginnt.
