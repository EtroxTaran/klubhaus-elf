---
title: "Pre-Mortem 2026-05-20 Â· 11 Â· AI/LLM Dependency & Fallbacks"
status: current
...
# Pre-Mortem 2026-05-20 Â· 11 Â· AI/LLM Dependency, Content Strategy & Fallback Architecture

> **Failure-Headline-Kandidaten**
> - â€žAnthropic sperrte Dev-Konto wegen ToS-Verdacht â€” 5 Tage Stillstand, Launch um 2 Wochen verschoben."
> - â€žOpus 4.7 Tokenizer-Update +35 % Kosten Ã¼ber Nacht â€” Monats-Burn â‚¬ 380 â†’ â‚¬ 510."
> - â€žUS-LLM-Anbindung schickte personalisierte Manager-Namen an OpenAI ohne EU-Endpoint â€” DSAR-Anfrage landete bei BfDI."
> - â€žEU AI-Act Art. 50 trat 2026-08-02 in Kraft â€” KI-generierte NPC-Dialoge ohne Kennzeichnung; Cease-and-Desist eines Compliance-Anwalts."
> - â€žDeepSeek V3.2 lieferte halluzinierte Spielerstatistiken im Audit-Explainer â€” Reddit-Screenshot 'Haaland 1987 geboren'."
> - â€žClaude Code Outage am Release-Weekend â€” 11 h, kein Fallback-Workflow."
...
### PM-2026-05-20-11-F-01 â€” Bus-Faktor â€žAnthropic" im Dev-Workflow
...
mitigation_summary: "SekundÃ¤rer BYOK-Pfad (Cursor Pro + Aider+OpenRouter); quarterly 'Tag ohne Claude Code'-Drill; lokales Qwen2.5-Coder-32B auf Hetzner GEX44 als Ultimate-Fallback"
linked_adrs: []
linked_specs: []
linked_code: []
sources:
...
**Hypothese.** Claude Code ist Single-Point-of-Failure unserer Dev-Velocity. Outage 24 h am Release-Wochenende = 1â€“3 Mannstage Verlust; Account-Ban = bis 5 Mannstage Re-Productivity auf alternativer Pipeline.

**Mitigation.** (1) Cursor Pro ($20/mo) + Aider mit OpenRouter-BYOK als Hot-Backup. (2) QuartÃ¤rlich â€žTag ohne Claude Code"-Drill. (3) Lokales `qwen2.5-coder:32b` (Ollama) auf GEX44 (â‚¬184/Mo, RTX 4000 Ada, 16,9 tok/s) als Ultimate-Offline-Fallback. (4) Alle Skills/Rules editor-agnostisch in `docs/90-Meta/`.

**Verifikation.** Drill-Protokoll mit Time-to-First-Productive-Commit. Ziel: < 4 h Switch-Cost zu Cursor, < 1 d zu Aider+local.

### PM-2026-05-20-11-F-02 â€” LLM leakt in Match-Engine-Pfad (Determinismus-Bruch)
...
**Hypothese.** Ohne harte Boundary wandert ein hilfreicher LLM-Call (â€žgenerate flavor string for goal") in den Engine-Pfad â†’ Replay-Determinismus weg â†’ Anti-Cheat tot, BYOC unmÃ¶glich, Art-50-Befreiung weg.

**Mitigation.** (1) ESLint `no-restricted-imports` in `packages/match-engine/**` fÃ¼r alle LLM-SDKs (`anthropic`, `openai`, `mistralai`, `@google/genai`, `ollama`) + HTTP-Clients. (2) DI-Container exponiert kein `LLMService` an Engine-Bounded-Context. (3) CI-Canary: Match-Engine-Determinism-Tests laufen mit `--no-net` Sandbox. (4) **ADR-0030 LLM-out-of-Authoritative-Game-State Boundary**.

**Verifikation.** CI-Job `match-engine-offline` schlÃ¤gt fehl bei Egress. Replay-Snapshot-Tests bit-identische Outputs.

### PM-2026-05-20-11-F-03 â€” Kosten-Explosion bei post-MVP Runtime-LLM
...
early_warning:
  - metric: "LLM cost per DAU"
    threshold: "> â‚¬ 0.02/DAU/mo bei einem Provider"
  - metric: "Daily token-spend"
    threshold: "> 2Ã— 7d-rolling-median"
mitigation_summary: "Mistral Small/Medium als Primary fÃ¼r DE-Generative; hard daily cost-cap; Template-Fallback IMMER vor LLM-Call"
linked_adrs: []
linked_specs: []
linked_code: []
sources:
...
**Hypothese.** Press-Conference-Texte via Opus 4.7 ($25/M out) bei 10k DAU Ã— 500 out-tok/Interaktion/Tag â‰ˆ $3.750/Mo. Mistral Small 3.1 ($0.60/M out): ~$90/Mo gleich.

**Mitigation.** (1) Hard cost-cap pro Use-Case (daily EUR-Cap in Redis-Counter); Ãœberschreitung â†’ 100 % Templates. (2) Default-Provider Mistral Small 3.1 fÃ¼r DE-Generative. (3) Aggressiv cachen (Anthropic Prompt Caching âˆ’90 %). (4) Batch-API wo Latenz egal (Inbox-Nightly): âˆ’50 %.

**Verifikation.** Cost-Canary 7-d-Synthetic-Load gegen jeden Provider, Vergleich EUR/User/mo. Grafana-Alert bei > â‚¬ 0.02/DAU/d.

### PM-2026-05-20-11-F-04 â€” DSGVO-VerstoÃŸ durch US-LLM ohne EU-Endpoint
...
**Hypothese.** Runtime-LLM mit PII (Spielername, Vereinsname, Mail) an US-Endpoint ohne ZDR â†’ BfDI-Beschwerde post-Schrems-II realistisches Reputations-/BuÃŸgeld-Risiko.

**Mitigation.** (1) **PII-Filter-Layer**: Eigennamen vor LLM-Call durch Token (`{{player_name}}`) ersetzen + Antwort rein-substituieren. (2) EU-only Default: **Mistral (Paris)** fÃ¼r PII-Use-Cases. (3) OpenAI nur via EU-Project + ZDR. (4) Anthropic nur via AWS Bedrock eu-central-1 oder Vertex Frankfurt. (5) DPA + SCC verlinkt im Trust-Center; AVV-Register-Eintrag pro Subprocessor.

**Verifikation.** Quarterly DPIA-Light. Network-Egress-Monitoring (Falco/eBPF): keine LLM-Egress auÃŸerhalb EU.

### PM-2026-05-20-11-F-05 â€” EU AI Act Art. 50 Kennzeichnungspflicht verpasst (Deadline 2026-08-02)
...
**Verifikation.** Unit-Test asserted: jeder AI-String trÃ¤gt `data-ai-generated="true"` + i18n-Label.

### PM-2026-05-20-11-F-06 â€” LLM-Halluzination im User-facing Output ohne Guard
...
early_warning:
  ...
  - metric: "Schema validation failures on LLM output"
    threshold: "> 5 %"
mitigation_summary: "Constrained generation (JSON-Schema); Post-Hoc-Fact-Check gegen Game-State; NeMo Guardrails / Llama-Guard fÃ¼r Toxicity; Template-Fallback bei Schema-Violation"
linked_adrs: []
linked_specs: []
linked_code: []
sources:
...
**Hypothese.** LLM fÃ¼r â€žErling Haaland"-Press-Conference halluziniert reale Trainingsdaten hinein (Trainingsdaten-Leak). Bei kreierten Spielernamen halluziniert es Stats.
...
### PM-2026-05-20-11-F-07 â€” Lokales LLM-Hosting (Hetzner) nicht ausreichend fÃ¼r synchrone Use-Cases
...
mitigation_summary: "Lokales LLM nur fÃ¼r asynchrone Use-Cases (Nightly-Inbox, Audit-Explainer); synchrone via Mistral-Cloud"
linked_adrs: []
linked_specs: []
linked_code: []
sources:
...
**Mitigation.** Local nur fÃ¼r async (Nightly-Inbox-Batch). Cloud (Mistral) fÃ¼r sync. GEX44 als Dev/Test-Fallback + Embedding-Server. Bei Production-Bedarf: Cluster 2Ã— GEX44 + LiteLLM-Load-Balancer.

### PM-2026-05-20-11-F-08 â€” Provider-Lock-In durch SDK-Spezifika
...
early_warning:
  - metric: "Direct LLM-SDK-calls outside LLM-adapter"
    threshold: "> 3 Stellen"
mitigation_summary: "Anti-Corruption-Layer: internes LLMService-Interface; OpenRouter oder LiteLLM als unified Backend fÃ¼r Failover"
linked_adrs: []
linked_specs: []
linked_code: []
sources:
...
### PM-2026-05-20-11-F-09 â€” Open-Source-Modell-Trainingsdaten-Lizenz-Risiko
...
**Mitigation.** Modell-Lizenz-Liste in ADR-0029: Apache-2.0/MIT/Llama-3-Community/Qwen-Apache. Vermeiden: Llama-2-non-commercial, â€žResearch use only". Bei Mistral/Anthropic/OpenAI: API-ToS reicht.

### PM-2026-05-20-11-F-10 â€” Cursor-Pricing-Schock: Claude Code Max heute gÃ¼nstigster Power-Plan
...
### PM-2026-05-20-11-F-11 â€” Halluzinierte Compliance-Antworten im Customer-Support-Bot
...
mitigation_summary: "Customer-Support-Bot post-MVP nur RAG mit fixem FAQ-Korpus; 'Ich bin AI; bei rechtlichen Fragen â†’ human'"
linked_adrs: []
linked_specs: []
linked_code: []
sources:
...
**Mitigation.** (1) RAG mit kuratiertem FAQ-Korpus (Markdown im Repo). (2) Hard-coded Topics (â€ždata-export", â€ždelete-account") routen direkt zu deterministischen Templates / Human-Hand-off. (3) Pflicht-Disclaimer â€žIch bin eine KI."

**Verifikation.** Adversarial-Testset 100 Fragen rund um DSGVO/Refund; halluzinierte Antwort = Bug-P0.

### PM-2026-05-20-11-F-12 â€” ToS-Verbot â€žAI-trained-on-AI" fÃ¼r Spielkontext
...
mitigation_summary: "Outputs nie als persistente Game-State (Determinismus erfÃ¼llt das schon); Logs nicht zum Re-Training"
linked_adrs: []
linked_specs: []
linked_code: []
sources:
...
**Mitigation.** Adapter-Layer protokolliert ToS-Version-Hash pro Provider; quartÃ¤rliche Re-Review. Provider-ToS-Snapshots in `docs/90-Meta/external-tos/`.
...
|---|---|---|---|---|---|---|---|
| Anthropic Claude Opus 4.7 | 5.00 | 25.00 | ~3â€“6 s (reasoning) | nur via AWS Bedrock eu-central-1 / Vertex EU | nein | Arena ~1500 (Top-3), Aider ~72 % | API ToS, DPA + SCC |
| Anthropic Claude Sonnet 4.6 | 3.00 | 15.00 | ~1â€“2 s | nur Bedrock/Vertex EU | nein | starkes Coding/Reasoning | API ToS |
| Anthropic Claude Haiku 4.5 | 1.00 | 5.00 | ~0.5â€“1 s | nur Bedrock/Vertex EU | nein | gut fÃ¼r Routing/Klassifikation | API ToS |
| OpenAI GPT-5.5 | 5.00 | 30.00 | ~1â€“2 s (Batch -50 %) | Per-Project EU + ZDR | nein | Arena Top-3 | API ToS |
| OpenAI GPT-5.4 | 2.50 | 15.00 | ~1â€“2 s | Per-Project EU | nein | Aider 88 % (high reasoning) | API ToS |
| OpenAI GPT-4o | 2.50 | 10.00 | ~0.8 s | Per-Project EU | nein | Arena ~1380 | API ToS |
| Google Gemini 2.5 Pro | 1.25 (<=200k) / 2.50 | 10.00 / 15.00 | ~1â€“2 s | Vertex AI EU-regions | AI Studio | Arena Top-3 | API ToS |
| Google Gemini 2.5 Flash | 0.30 | 2.50 | ~0.4 s | Vertex AI EU | ja, generÃ¶s | gut High-Volume | API ToS |
| **Mistral Large 3** | 2.00 | 6.00 | ~1â€“2 s | **Paris EU nativ** | Experiment | strong, EU-best | API ToS |
| **Mistral Medium 3** | 0.40 | 2.00 | ~0.8 s | **Paris EU** | Experiment | gut fÃ¼r DE-Texte | API ToS |
...
| Local Ollama Qwen2.5-Coder 32B (GEX44) | selfhost ~â‚¬184/Mo | selfhost | ~17 tok/s @ 1 user | **ja, self** | n/a | nahe GPT-4o im Coding | Apache-2.0 |
| Local Llama 3.3 70B (48GB+) | selfhost | selfhost | ~5â€“12 tok/s | **ja, self** | n/a | strong general | Llama Community |
| Local Phi-3 mini / Qwen2.5-7B (CCX-CPU) | selfhost | selfhost | 1â€“2 tok/s | **ja, self** | n/a | nur kleine Tasks | MIT/Apache |
...
## Use-Case Ã— Provider Recommendation Matrix (CORE OUTPUT)

Annahme 10k DAU. USD/Mo.

| Use-Case | Primary | Secondary | Templates Fallback | Cost @ 10k DAU/Mo |
|---|---|---|---|---|
| Dev workflow heute | Claude Code (Anthropic Pro $20) | Cursor Pro ($20), Aider+OpenRouter, Continue+Ollama | n/a | $20â€“125/seat |
| Press-Conference (post-MVP) | Mistral Small 3.1 (EU) | Mistral Medium 3, Gemini 2.5 Flash | **ja** (Markdown+ICU) | ~$25â€“90 (batch+cache) |
| NPC manager dialogue | Mistral Medium 3 (EU) | Anthropic Haiku 4.5 Bedrock-EU | **ja** canned per persona | ~$80â€“200 |
| Inbox dynamic copy | Mistral Small 3.1 (batch âˆ’50 %) | Gemini 2.5 Flash | **ja** 80â€“120 ICU-Templates | ~$15â€“40 (nightly) |
| Customer-Support Chat | Mistral Small 3.1 + RAG (EU) | OpenAI GPT-4o EU-Project | **ja** statisches FAQ + Hand-off | ~$30â€“80 |
...
| Game-State-Explainer | Mistral Medium 3 (EU) | Gemini 2.5 Flash | **ja** rule-based | ~$20â€“50 |
| **NEVER** Match-Engine | â€“ | â€“ | **only deterministic** | 0 (must be) |
| **NEVER** Player-Attribute-Generation | â€“ | â€“ | **only PCG32 from seed** | 0 (must be) |
| **NEVER** Authoritative Game-State | â€“ | â€“ | **only deterministic** | 0 (must be) |

## Fallback-Chain-Architecture (CORE OUTPUT)

```
Request â†’ Cost-Cap-Counter â†’ Cache-Lookup â†’ Circuit-Breaker
  â†’ Primary: Mistral Small 3.1 (EU)         [800ms timeout]
  â†’ Secondary: Mistral Medium 3 (EU)        [1500ms]
  â†’ Tertiary: Gemini 2.5 Flash via Vertex EU [1500ms]
  â†’ Templates (ICU deterministic, always succeeds)
...
Post-LLM Guards:
  ...
  Any fail â†’ Templates

AI-disclosure label (Art. 50) â†’ Display
```

Circuit-Breaker (Hystrix): Window 60 s, threshold 50 % failures Ã¼ber >=10 Requests â†’ open. Half-open Probe every 30 s; 3 consecutive successes â†’ close.

Time-Budget: 800 ms sync user-facing; 30 s async batch (Inbox-Nightly).

Cost-Cap pro Use-Case/Tag: Press-Conf â‚¬5, NPC â‚¬8, Support â‚¬3, Translation â‚¬2, Audit â‚¬0 (lokal).

## EU AI Act Compliance Assessment

- **Risk class**: text-only Manager-Sim mit deterministischen Templates = **minimal risk**. LLM-Runtime fÃ¼r End-User-Outputs = **limited risk** mit Art-50-Transparenz-Pflicht.
...
- **GPAI**: wir konsumieren, trainieren nicht â†’ wir sind â€žDeployer". Article 4 (AI Literacy seit 2025-02-02) gilt: Founder + Team brauchen nachweisbar AI-Literacy (Onboarding-Modul â€žLLM-Risiken").
- **MVP-Status mit Determinismus + Templates befreit uns weitgehend** â€” starkes Argument fÃ¼r Templates-First.
...
| **Total Mistral-Stack** | â€“ | â€“ | â€“ | â€“ | **~$60/Mo** | **~$2.000/Mo** | **~$170/Mo** |

Bei 30 % Cache-Hit-Rate + Prompt-Caching (âˆ’90 % Anthropic / âˆ’50 % Mistral cached input): Mistral-Stack realistisch **~$30â€“40/Mo**.

**Break-Even local vs cloud**: GEX44 â‚¬184/Mo + Setup vs $60/Mo Mistral-Stack. Cloud klar billiger; lokal lohnt erst > $300/Mo Cloud-Spend ODER bei strenger EU-Residency-Notwendigkeit. **Cloud-first via Mistral, lokal nur sensitive async.**

**Latenz-Budget sync 800 ms total**: Network EUâ†”Paris ~30â€“50 ms + Mistral Small Inference ~300â€“400 ms (200 out tok) + Guards ~30 ms + Cache ~5 ms = ~300 ms Reserve.

## SLO-VorschlÃ¤ge

1. **LLM response latency P99 â‰¤ 1.500 ms** (oder Templates-Fallback).
2. **Fallback-rate (LLMâ†’Templates) â‰¤ 5 %** monatlich; bei > 5 % Provider-Health-Review.
3. **Daily-cost-cap per use-case** (EUR-fixed); > 100 % â†’ Templates erzwungen.
4. **Hallucination-detected rate â‰¤ 1 %** (schema-fail + Golden-Set-Spot-Check).
5. **Provider-availability â‰¥ 99,5 %** (rolling 30d) â€” bei < 99 % SekundÃ¤r als Default.
...
3. Bei Runtime-Outage: Circuit-Breaker Ã¶ffnet automatisch â†’ Mistral Medium (sekundÃ¤r) oder Templates.
4. Cost-Bookkeeping: SekundÃ¤r-Provider-Spike 4h OK, > 4h Templates-Only erzwingen.
...
4. Legitime Demand â†’ Cap raisen (Founder-Approval). Token-Drift â†’ Provider-Wechsel.

### RB-11-C: Halluzination in Production
1. User-Report / Schema-Validation-Fail-Alert.
2. Sofort Feature-Flag `llmEnabled=false` â†’ Templates-Fallback.
...
1. Wann darf Mistral statt Templates greifen? Konkrete Trigger pro Use-Case (z.B. â‰¥ 3 Templates â†’ rotation; < 3 â†’ LLM-augment).
2. Anthropic Bedrock EU vs Mistral nativ bei ~50â€“100 USD/Mo Spend (AWS-Account-Overhead vs Mistral-direct)?
3. **AI-Literacy-Modul (Art. 4)** â€” inhouse oder externes Training? DE-Sprachiges Modul?
4. Lokales LLM (Ollama) als Privacy-Anchor fÃ¼r Audit-Explainer â€” lohnt GEX44 (â‚¬184/Mo) auch ohne hohen Cloud-Spend? Alt: Modal/Replicate Spot-on-Demand.
5. Player-data-Redaction-Policy: welche Felder nie an Server, welche nur EU-Hosts, welche US mit ZDR? â†’ separates **ADR-0031** mit DPO-Sign-Off.

## "Wenn wir nur 3 Dinge tun"-Liste

1. **MVP bleibt LLM-frei in Runtime.** Markdown + ICU-Templates fÃ¼r 100 % User-facing Texte. Spart Cost + KomplexitÃ¤t + Determinismus-Risiko + AI-Act-Pflichten. (Highest leverage; aligned mit Narrative-Pipeline-Decision.)
2. **LLMService-Adapter + Mistral-as-default + Fallback-Chain bauen, sobald erster post-MVP-LLM-Use-Case kommt.** Kein direkter SDK-Call auÃŸerhalb `packages/llm-adapter/`. Erzwingt Provider-Agnostik, EU-Residency-Default, Cost-Cap, Guards.
3. **Dev-Tool-Bus-Factor-Drill quartÃ¤rlich.** 1 Sprint-Woche ohne Claude Code (Cursor + Aider + Ollama-Backup). Garantiert Recovery < 1 d.
...
- [[PM-2026-05-20-13-community-moderation-and-ugc]] (Llama-Guard fÃ¼r Moderation)
- [[../narrative-content-pipeline]] Â· [[../ai-manager-behaviour]]
- [[../../10-Architecture/09-Decisions/ADR-0003-match-engine]]
