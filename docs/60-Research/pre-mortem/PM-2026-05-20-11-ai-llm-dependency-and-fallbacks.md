...
> - „Anthropic sperrte Dev-Konto wegen ToS-Verdacht — 5 Tage Stillstand, Launch um 2 Wochen verschoben."
> - „Opus 4.7 Tokenizer-Update +35 % Kosten über Nacht — Monats-Burn € 380 → € 510."
> - „US-LLM-Anbindung schickte personalisierte Manager-Namen an OpenAI ohne EU-Endpoint — DSAR-Anfrage landete bei BfDI."
> - „EU AI-Act Art. 50 trat 2026-08-02 in Kraft — KI-generierte NPC-Dialoge ohne Kennzeichnung; Cease-and-Desist eines Compliance-Anwalts."
> - „DeepSeek V3.2 lieferte halluzinierte Spielerstatistiken im Audit-Explainer — Reddit-Screenshot 'Haaland 1987 geboren'."
> - „Claude Code Outage am Release-Weekend — 11 h, kein Fallback-Workflow."
...
### PM-2026-05-20-11-F-01 — Bus-Faktor „Anthropic" im Dev-Workflow
...
**Mitigation.** (1) Cursor Pro ($20/mo) + Aider mit OpenRouter-BYOK als Hot-Backup. (2) Quartärlich „Tag ohne Claude Code"-Drill. (3) Lokales `qwen2.5-coder:32b` (Ollama) auf GEX44 (€184/Mo, RTX 4000 Ada, 16,9 tok/s) als Ultimate-Offline-Fallback. (4) Alle Skills/Rules editor-agnostisch in `docs/90-Meta/`.
...
**Hypothese.** Ohne harte Boundary wandert ein hilfreicher LLM-Call („generate flavor string for goal") in den Engine-Pfad → Replay-Determinismus weg → Anti-Cheat tot, BYOC unmöglich, Art-50-Befreiung weg.
...
**Mitigation.** (1) Hard cost-cap pro Use-Case (daily EUR-Cap in Redis-Counter); Überschreitung → 100 % Templates. (2) Default-Provider Mistral Small 3.1 für DE-Generative. (3) Aggressiv cachen (Anthropic Prompt Caching −90 %). (4) Batch-API wo Latenz egal (Inbox-Nightly): −50 %.
...
**Hypothese.** LLM für „Erling Haaland"-Press-Conference halluziniert reale Trainingsdaten hinein (Trainingsdaten-Leak). Bei kreierten Spielernamen halluziniert es Stats.
...
**Mitigation.** Modell-Lizenz-Liste in ADR-0029: Apache-2.0/MIT/Llama-3-Community/Qwen-Apache. Vermeiden: Llama-2-non-commercial, „Research use only". Bei Mistral/Anthropic/OpenAI: API-ToS reicht.
...
**Mitigation.** (1) RAG mit kuratiertem FAQ-Korpus (Markdown im Repo). (2) Hard-coded Topics („data-export", „delete-account") routen direkt zu deterministischen Templates / Human-Hand-off. (3) Pflicht-Disclaimer „Ich bin eine KI."

**Verifikation.** Adversarial-Testset 100 Fragen rund um DSGVO/Refund; halluzinierte Antwort = Bug-P0.

### PM-2026-05-20-11-F-12 — ToS-Verbot „AI-trained-on-AI" für Spielkontext
...
| Inbox dynamic copy | Mistral Small 3.1 (batch −50 %) | Gemini 2.5 Flash | **ja** 80–120 ICU-Templates | ~$15–40 (nightly) |
...
- **GPAI**: wir konsumieren, trainieren nicht → wir sind „Deployer". Article 4 (AI Literacy seit 2025-02-02) gilt: Founder + Team brauchen nachweisbar AI-Literacy (Onboarding-Modul „LLM-Risiken").
...
Bei 30 % Cache-Hit-Rate + Prompt-Caching (−90 % Anthropic / −50 % Mistral cached input): Mistral-Stack realistisch **~$30–40/Mo**.
