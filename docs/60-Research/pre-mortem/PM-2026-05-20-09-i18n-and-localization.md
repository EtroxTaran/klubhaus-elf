---
title: "Pre-Mortem 2026-05-20 Â· 09 Â· i18n & Localization"
status: current
...
# Pre-Mortem 2026-05-20 Â· 09 Â· Internationalization & Localization

> **Failure-Headline-Kandidaten**
> - â€žPL-Plural-Bug 'Du hast 1 Mitteilungen' â€” beim ersten polnischen Streamer-Auftritt â€” Vertrauen weg."
> - â€žDE-User sehen 50 % der Texte als schlechte MT â€” DeepL-Fast-Track Ã¼berspringt Glossar, Markenbegriffe falsch."
> - â€žRTL-Layout bricht bei Tabellen, weil 4 von 87 Komponenten noch `ml-2`/`pr-4` statt `ms-2`/`pe-4` benutzen."
> - â€žBundle-Bloat: Initial-Load zieht alle 8 Locale-Bundles Ã  120 KB â€” Lighthouse fÃ¤llt von 92 auf 51."
> - â€žRU-UI zeigt `Bayern MÃ¼nchen Ð² Ð¼Ð°Ñ‚Ñ‡Ðµ Ð¿Ñ€Ð¾Ñ‚Ð¸Ð² Borussia Dortmund` ohne Genitiv `Ð‘Ð°Ð²Ð°Ñ€Ð¸Ð¸` â€” fÃ¼hlt sich Maschinen-generiert an."

## Scope

i18n/l10n als Architektur-Pre-Mortem: ICU-MessageFormat-Coverage, React-i18n-Lib-Wahl, RTL-Readiness, TMS-Workflow, Pseudo-Localization, Datum/WÃ¤hrung, Sprach-Fallback-Kette, Unicode-Validation, Schriftarten-Subsetting, Locale-Detection, SEO/hreflang, Ãœbersetzer-Onboarding, Service-Worker-Caching. Aufbauend auf gelocktem [[../narrative-content-pipeline]] (ICU-MF1, deterministische Templates, 80â€“120 MVP, 95â€“145 KB gz/Locale).

**Welle-1+2-Decisions**: MVP LLM-frei in Runtime (keine LLM-Ãœbersetzung), 5 MVP-Locales empfohlen (DE/EN/FR/ES/IT).

## Top Failure-Hypothesen

### PM-2026-05-20-09-F-01 â€” ICU MessageFormat reicht fÃ¼r 90 %, scheitert bei flektierten Klubnamen
...
**Hypothese.** ICU lÃ¶st Plural/Gender/Select. Nicht Flexion von Eigennamen in Slawischen Sprachen und Deutsch. â€ž1 Spieler von Bayern MÃ¼nchen" trivial im DE, im RU braucht Klubname Genitiv (`Ð¸Ð· Ð‘Ð°Ð²Ð°Ñ€Ð¸Ð¸ ÐœÑŽÐ½Ñ…ÐµÐ½`), im PL Akkusativ. Daten-Modell speichert nur `clubName: "Bayern MÃ¼nchen"` â†’ jede locale-spezifische Konjugation = tote Strings.

**Mitigation.** Club-Schema: optionales `localizedNames: { de: {nom, gen, dat, akk}, ru: {...}, pl: {...} }`. ICU-Strings nur Ã¼ber Casus-System: `{club_gen}` statt `{club}`. Translator-Glossar fixiert Top-100-Klubs alle Casus manuell. Build-Time-Tool (DeepL Pro + Glossar) generiert VorschlÃ¤ge, Human-Review. MF2 als 2027-Item.

**Verifikation.** Snapshot-Test: fÃ¼r jedes ICU-Template 20 zufÃ¤llige SÃ¤tze mit Top-5-Liga-Klubs; Linter prÃ¼ft alle `{club_*}`-Slots. CI-Job `pnpm i18n:icu-validate` parst mit `@formatjs/icu-messageformat-parser`.

### PM-2026-05-20-09-F-02 â€” Library-Lock-in: Paraglide gewinnt im Bundle-Budget, react-intl ist ICU-Standard
...
mitigation_summary: "Paraglide JS fÃ¼r UI-Strings + format.js Intl-Polyfills fÃ¼r Numbers/Dates; Library-Entscheidung vor Sprint-2"
linked_adrs: []
...
**Hypothese.** 80â€“120 ICU-Templates Ã— 5â€“10 Locales: Runtime-Lookup-Libs (i18next, react-intl) schwellen Initial-Bundle. Paraglide kompiliert zu tree-shakable ESM, kann aber kein dynamisches Locale-Hot-Swap ohne Reload.

**Mitigation.** Hybride Architektur: **Paraglide** fÃ¼r 90 % UI-Strings (compile-time, tree-shake, type-safe). **format.js Intl** (PluralRules, DateTimeFormat, NumberFormat, RelativeTimeFormat) fÃ¼r Formatter â€” moderne Browser haben sie nativ. Sprachwechsel via `window.location.reload()` (akzeptabel selten). Migration vor Sprint-2.

**Verifikation.** `pnpm build:analyze` Budget: i18n-Lib-Runtime â‰¤ 8 KB gz, Initial-Locale-Bundle â‰¤ 25 KB.

### PM-2026-05-20-09-F-03 â€” RTL-Bruchstellen ohne enforced Logical-Properties-Migration
...
early_warning:
  - metric: "stylelint_logical_props_violations"
    threshold: "> 0 in CI"
mitigation_summary: "Lint-Rule verbietet ml-*/mr-*/pl-*/pr-* â€” nutzt ms-*/me-*/ps-*/pe-*; Storybook RTL-Toggle; Playwright fÃ¤hrt Visual-Tests 2Ã— LTR/RTL"
linked_adrs: []
...
**Hypothese.** Auch wenn MVP-Scope kein Arabisch enthÃ¤lt: heute mit `ml-/pr-`-Klassen geschriebene Komponente = Refactor-Schulden, die spÃ¤ter exponentiell wachsen. Gemischte Spielernamen (lateinisch in arabischem Satz) brauchen `<bdi>` / FSI/PDI-Unicode-Isolation.

**Mitigation.** (1) `eslint-plugin-tailwindcss` custom rule `no-logical-direction-classes`. (2) React-Helper `<Name lang="lat">{name}</Name>` setzt `<bdi>`. (3) Pseudo-RTL-Mode (`dir="rtl"` + `[a-zA-Z]â†’[ï»²ï»³ï»¯]`) via `?pseudo=rtl`. (4) Logical Properties heute schreiben â€” Mehraufwand ~5 %, Refactor ohne RTL-Prep > 30 %.

**Verifikation.** CI `pnpm test:rtl-snapshots` + `pnpm lint:logical-props`.

### PM-2026-05-20-09-F-04 â€” Pseudo-Localization fehlt im CI â†’ Layout-Bugs Ã¼berleben bis Beta
...
early_warning:
  - metric: "max_string_expansion_ratio_de_en"
    threshold: "> 1.6 (DE > 60 % lÃ¤nger als EN â†’ Buttons overflow)"
mitigation_summary: "Pseudo-Locale qps-ploc als Build-Target, Storybook-Toggle, Playwright-CI-Run; Stylelint-Defaults fÃ¼r truncation"
linked_adrs: []
linked_specs: []
...
**Hypothese.** DE ~30 % lÃ¤nger als EN, FI/PL teils 40 %, HU bis +60 %. â€žSave" â†’ â€žAufstellung speichern" â†’ â€žTallenna joukkueasetelma" sprengt Buttons. Ohne Pseudo-Locale erst beim DE-Beta-Tester sichtbar.

**Mitigation.** Build-Script `pnpm i18n:pseudo` generiert `qps-ploc.json` durch Char-Mapping + Padding (Ã— 1.4) + `[!! ... !!]`. Storybook-Toggle. Playwright-Visual-Test. Plus `qps-rtl` fÃ¼r RTL-Smoke ohne Arabisch-Ãœbersetzung zu pflegen.

**Verifikation.** Playwright-Snapshot pro Critical-Page in `qps-ploc`; Overflow-Bug-Count im PR-Diff: 0.

### PM-2026-05-20-09-F-05 â€” Validation-Regexe `[a-zA-Z]` blockieren legitime EU-User
...
**Hypothese.** â€žMÃ¼ller-GÃ¶tze", â€žSaint-Ã‰tienne", â€žÃ–stersunds saison '24" fallen bei naive `[a-zA-Z0-9 ]`-Patterns durch. DE-First-Market mit Umlauten/ÃŸ Standard.

**Mitigation.** Zentrale `src/lib/validation/names.ts` mit `\\p{L}\\p{M}\\p{N}\\p{Pd}\\p{Zs}.'` + length-cap. Reject `\\p{Cc}`, `\\p{Cf}` (auÃŸer ZWJ), RLO/LRO/RLE/LRE/PDF (Spoofing). Unit-Test mit Test-Corpus aus 50 DE/FR/PL/RU-Namen + 20 Spoof-Versuchen.

**Verifikation.** Snapshot-Test `validation.names.test.ts`. Continuous-Eval gegen `top-1000-european-names`-Dataset.

### PM-2026-05-20-09-F-06 â€” TMS-Lock-in: Crowdin/Lokalise-Pricing skaliert mit Wachstum
...
**Mitigation.** **Tolgee self-hosted** auf Hetzner CX22 (~â‚¬ 5/Monat): ICU-MessageFormat-native, In-Context-Editing in PWA, MCP-Server, Apache-2.0. Crowdin Free als Backup. Glossar + TM zusÃ¤tzlich in `docs/15-i18n/glossary.csv` versioniert.

**Verifikation.** DR-Drill: jeden Sprint einmal Export aller Ãœbersetzungen aus Tolgee als XLIFF, Import in Crowdin Sandbox, ICU-Syntax-Vergleich.

### PM-2026-05-20-09-F-07 â€” Locale-Detection-Falle: Cookies vs URL vs Accept-Language inkonsistent
...
mitigation_summary: "URL-First-Strategy (/de/, /en/) mit Cookie-Persist; Accept-Language nur Hint; nie IP-Geo fÃ¼r Sprach-Wahl (GDPR); hreflang + Sitemap pro Locale"
linked_adrs: []
...
**Hypothese.** `/de/dashboard` von Browser mit `Accept-Language: en` ohne Cookie: Sprache eindeutig DE (URL gewinnt). Ohne Hierarchie-Schema â†’ Hydration-Mismatch (Server EN, Client switcht DE) â†’ Flicker + SEO-Confusion.
...
**Verifikation.** Playwright-Test mit 16 Kombinationen (URL Ã— Cookie Ã— Accept-Language Ã— User-Override). Google-Search-Console-Sitemap-Validator monthly.

### PM-2026-05-20-09-F-08 â€” Schriftarten-Bloat bei CJK/Arabic erschlÃ¤gt Performance-Budget
...
**Verifikation.** Lighthouse-CI per locale-Build prÃ¼ft `total-byte-weight`. Slow-3G in DevTools alle Locales testen, LCP < 2.5 s.

### PM-2026-05-20-09-F-09 â€” Ãœbersetzer-Onboarding fehlt: Tonfall, Glossar, Don't-Translate-Liste undefiniert
...
mitigation_summary: "Style-Guide docs/15-i18n/style-guide.md mit Du/Sie (Du fÃ¼r Gaming), Glossar Top-200 in 5â€“8 Locales, Don't-Translate-Liste, Tonfall-Beispiele"
linked_adrs: []
linked_specs: [[[../narrative-content-pipeline]]]
linked_code: []
sources:
...
**Mitigation.** Sprint-1: 4-Seiten-`style-guide.md`: (1) **Du-Form DE** (Casual-Gaming-Norm). (2) Glossar-CSV Top-200 Football-Begriffe in 8 Locales. (3) Don't-Translate: Klubnamen Default nominativ, Liga-Codes (BL1/EPL/LaLiga), Tactical-Terms (Tiki-Taka). (4) Tonfall-Beispiele pro Variante. DeepL-Glossar lÃ¤dt CSV als Termbase.

**Verifikation.** QuartÃ¤rlicher Translation-QA-Audit: 50 zufÃ¤llige Strings pro Locale gegen Style-Guide. Score > 90 %.

### PM-2026-05-20-09-F-10 â€” Service-Worker serviert veralteten Locale-Bundle nach Hotfix
...
**Mitigation.** Vite generiert Hash-suffixed Locale-Files. `i18n-manifest.json` (~1 KB) mappt locale-code â†’ Hash-URL, `cache-control: max-age=300`. Bundle-Dateien `cache-control: max-age=31536000, immutable`. SW pre-fetcht aktuelle Locale aus Manifest-Refresh; bei Mismatch Background-Sync + notify().

**Verifikation.** E2E: SW-Cache fÃ¼llen, neue Hash deployen, Reload, prÃ¼fen ob neue Strings geladen. SLO: > 95 % User auf neuer Version < 60 min.
...
| **Weblate** | GPL-3 | ja | fÃ¼r libre projects | â‚¬45/Mo Cloud | partial | sehr stark | nein | ja |
...
- Narrative-Content (per Spec): 95â€“145 KB gz/Locale.
- Schrift-Subsets: 30â€“50 KB pro Family.
- **Initial-Load (1 Locale)**: ~58 KB extra (10 KB i18n-Lib + 8 KB Strings + 40 KB Font).
- **Worst-Case Narrative-Prefetch**: +120 KB â†’ ~178 KB â€” Lazy-Load via Loader.
- Ãœbersetzungs-Kosten (Mai 2026): pro 1k WÃ¶rter DEâ†”EN: â‚¬100â€“180 professional / â‚¬40â€“70 augmented (DeepL+Human) / DeepL Pro API frei fÃ¼r MVP ($25/Mo / 1M chars).
- MVP-Ãœbersetzungs-Budget fÃ¼r 5 Locales Ã— 8 k WÃ¶rter Ã— â‚¬ 0.05 = **â‚¬ 1.750 augmented** (vs â‚¬ 5.600 professional).
- Tolgee â‚¬5â€“10/Mo + DeepL Pro $25/Mo = **~â‚¬30/Mo Total**.
...
| Translation-Hotfix-Propagation | â‰¥ 95 % User auf neuer Version < 60 min |
| hreflang-Hygiene | 0 Sitemap-Cluster-Errors in Search Console |

## Test-Plan

- Pseudo-Loc (jeder PR, blocking): Build-Plugin â†’ `qps-ploc.json`; Storybook + Playwright + Visual-Regression.
...
- Plural-Rule-Coverage: Snapshot pro ICU-Plural mit Eingaben [0,1,2,3,5,11,21,100] pro Locale. PL/RU/CZ â†’ alle Kategorien (one/few/many/other).
- Locale-Detection-Matrix (E2E): 16 Kombinationen.
- Hreflang-Sitemap-Bot (Monthly).

## Runbook-Skizzen

### RB-09-A: Eilige Ãœbersetzungs-Korrektur Live
1. Fix in `de.json` (z.B. "Vetrag" â†’ "Vertrag").
2. CI: ICU-Validate + Pseudo-Loc + Visual-Regression.
3. Merge â†’ Vite Build erzeugt `de.<NEW_HASH>.json`.
...
3. Pre-Commit `i18n:icu-validate` prÃ¼ft CLDR-Kategorien-Match.
4. Storybook-Snapshot grÃ¼n â†’ Hotfix-Release wie RB-09-A.

### RB-09-C: Fallback-Chain-Failure (NO-Ãœbersetzung fehlt, fÃ¤llt auf EN statt SE)
1. Telemetry: `missing_translation_key_count` pro Locale-Chain.
2. Fallback-Konfig: `no â†’ se â†’ en`, `da â†’ no â†’ se â†’ en`, `et â†’ fi â†’ en`.
3. Per PR Default-Locale-Map aktualisieren; Stille-Fallback (kein UI-Banner â€” User soll nicht merken).
...
4. **Tolgee vs Crowdin?** HÃ¤ngt von FMX-Lizenz-Strategie (Cross-Ref Report 18 â€” AGPL-3.0 empfohlen â†’ Crowdin OSS-Tier eligible).
...
3. **Tolgee self-hosted + DeepL Pro + Glossar-CSV** (F-06). â‚¬30/Mo Total-Cost, MCP-Server-Integration, kein Vendor-Lock-in.
...
- [[PM-2026-05-20-10-accessibility-and-inclusion]] (Cross-Ref: Unicode Validation + RTL Ã¼berlappend mit a11y)
- [[PM-2026-05-20-13-community-moderation-and-ugc]] (Cross-Ref: Unicode-aware Hate-Speech-Filter)
- [[../narrative-content-pipeline]] Â· [[../performance-budgets]]
