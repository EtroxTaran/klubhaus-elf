...
> - ”žPL-Plural-Bug 'Du hast 1 Mitteilungen' — beim ersten polnischen Streamer-Auftritt — Vertrauen weg."
...
**Mitigation.** (1) `eslint-plugin-tailwindcss` custom rule `no-logical-direction-classes`. (2) React-Helper `<Name lang="lat">{name}</Name>` setzt `<bdi>`. (3) Pseudo-RTL-Mode (`dir="rtl"` + `[a-zA-Z]→[ï»²ï»³ï»¯]`) via `?pseudo=rtl`. (4) Logical Properties heute schreiben — Mehraufwand ~5 %, Refactor ohne RTL-Prep > 30 %.
...
**Hypothese.** DE ~30 % länger als EN, FI/PL teils 40 %, HU bis +60 %. ”žSave" → ”žAufstellung speichern" → ”žTallenna joukkueasetelma" sprengt Buttons. Ohne Pseudo-Locale erst beim DE-Beta-Tester sichtbar.
...
**Hypothese.** ”žMüller-Götze", ”žSaint-Étienne", ”žÖstersunds saison '24" fallen bei naive `[a-zA-Z0-9 ]`-Patterns durch. DE-First-Market mit Umlauten/ß Standard.
