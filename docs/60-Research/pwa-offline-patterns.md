...
- It maps cleanly onto the Postgres per-save schema model: opaque
  branded UUID refs for relationships and junction tables for edges
  with lifecycle, one schema per save
  ([[../10-Architecture/09-Decisions/ADR-0027-postgres-data-model]] §1, §5, §6).
