...
> [[../00-Index/MVP-Scope]], [[../50-Game-Design/GD-0017-mvp-scope-and-mode-sequencing]]
...
> [[../50-Game-Design/GD-0017-mvp-scope-and-mode-sequencing]].** The bullets
> below preserve the original research outcome. Current MVP: Roguelite active,
> Career "comes later", hybrid-online (see [[../10-Architecture/09-Decisions/ADR-0020-hybrid-online-mvp-offline-ready]]).
...
- **Q&A outcome (2026-05-17)** — Perplexity research (CWV / Lighthouse, device matrix, JS budgets, test-rig comparison, comparative analysis of FM Mobile / Top Eleven / OSM / SM24 / Hattrick / EA FC Mobile / others) + Nico:
  ...
  - **Match render policy**: explicit no interactive/authoritative browser 3D match view. The original D9 phrasing was "no 3D ever"; [[../10-Architecture/09-Decisions/ADR-0029-3d-presentation-layer]] scoped that ban to the live match render pipeline on 2026-05-20, and [[../10-Architecture/09-Decisions/ADR-0041-presentation-renderer-strategy]] tightened the renderer portfolio on 2026-05-22. Two modes only - Text & Stats (first-class, Floor default) + 2D canvas (primary, mandatory, 30/60 fps cap by tier). The separate 3D Presentation Layer is Three.js/R3F only and must remain outside the match renderer; PixiJS is no longer planned for the match surface.
...
- **Output files**: [[threat-model]] (new, `status: current binding`); updates [[../00-Index/Decision-Log]], [[../00-Index/Current-State]], C6 / C8 / D18 entries (annotated as anchored on F1).
...
- **Priority**: P0.
- **Q&A outcome (2026-05-18, Perplexity-backed)** — six focused sonar-pro queries (passkey state across browsers, ASVS v5 + NIST 800-63B AAL2 anchors, cookies + tokens in 2026, per-flow specs, OAuth/social login posture, throttling + step-up + anomaly signals) feed [[../30-Implementation/auth-flows]]. Locked decisions:
  ...
  - **7 product Q&A questions** surfaced for Nico (account-master-key envelope timing, email verification gating, MFA mandatory vs opt-in, "cannot recover" confirmation, CAPTCHA posture, SMTP provider pick [Brevo default], magic-link as third login option). Listed in [[../30-Implementation/auth-flows]] §11.
  - **9 follow-up tasks** (FU-1..FU-9) anchored to F3/F5/F6/F12/E10/E11.
- **Output files**: [[../30-Implementation/auth-flows]] (new, `status: current binding`); updates [[../00-Index/Decision-Log]], [[../00-Index/Current-State]], [[../00-Index/Implementation-Map]]; refines ADR-0005 §3 with the bootstrap endpoint contract (non-breaking).
- **Done outputs**: [[../30-Implementation/auth-flows]] (current binding).
...
- **Q&A outcome (2026-05-18, Perplexity-backed)** — six focused sonar-pro queries (Redis session schema 2026, refresh-token rotation + race handling, idle/absolute lifetimes for offline-first PWAs, device list UX + data model, full revocation runbook, TanStack Start SSR integration) feed [[../30-Implementation/session-management]]. Locked decisions:
  ...
  - **7 product Q&A questions** surfaced for Nico (15-second grace window, drop "Remember me" checkbox, idle/absolute numerics, SSE push deferral, "Trust this device" opt-in, device-revoke does not rotate accountSecret, admin CLI not UI at MVP). Listed in [[../30-Implementation/session-management]] §15.
  - **9 follow-up tasks** (FU-1..FU-9) anchored to E10/E11/F3 post-MVP/F5/F6/F11/F12.
- **Output files**: [[../30-Implementation/session-management]] (new, `status: current binding`); updates [[../00-Index/Decision-Log]], [[../00-Index/Current-State]], [[../00-Index/Implementation-Map]]; closes F2 FU-2 (CI lint rule for `authorize()`).
- **Done outputs**: [[../30-Implementation/session-management]] (current binding).
...
- **Q&A outcome (2026-05-18, Perplexity-backed)** — five focused sonar-pro queries (industry envelope-design comparison across Bitwarden / 1Password / Standard Notes / Proton / Signal / iCloud Keychain / WhatsApp; Web Crypto API mechanics for non-extractable wrap/unwrap; rotation algorithm + race handling + F2→F5 migration; recovery flow + attack mitigation matrix; multi-device continuity post-rotation) feed [[../30-Implementation/account-recovery]]. **Adopted Option A** from the A5 question (stable account-key in platform DB, wrapping key rotates). Locked decisions:
  ...
  - **6 product Q&A questions** surfaced for Nico (canonical Env_user architecture; one-shot save re-encryption during migration; userSalt rotation only on recovery-code path; password reset rotates accountSecret; recovery codes regenerated only on recovery-code use; full AAD layout). Listed in [[../30-Implementation/account-recovery]] §13.
  - **9 follow-up tasks** (FU-1..FU-9) anchored to F4 / F6 / E10 / E11 / post-MVP.
- **Output files**: [[../30-Implementation/account-recovery]] (new, `status: current binding`); updates [[../00-Index/Decision-Log]], [[../00-Index/Current-State]], [[../00-Index/Implementation-Map]]; closes F2 FU-1 (stable account-master-key envelope) and F3 FU-7 (stable account-master-key envelope decoupling rotation from device-backup save key); refines ADR-0005 §3 with the envelope path (additive — old direct-derive path stays readable during the lazy migration window).
- **Done outputs**: [[../30-Implementation/account-recovery]] (current binding).
...
- **Output files**: [[gdpr-compliance]] (new research synthesis, `status: current binding`) + [[../30-Implementation/privacy-and-consent]] (new implementation note, `status: current binding`); updates [[../00-Index/Decision-Log]], [[../00-Index/Current-State]], [[../00-Index/Implementation-Map]], [[../00-Index/Research-Map]]; **closes F2 FU-6 + F2 FU-7 + F3 FU-8 + F5 FU-8 + F5 FU-9** by absorbing the DSAR scope, the DPIA on `accountSecret` + WebAuthn + master-key envelope, the device + auth_events DSAR inclusion, and the lawful-basis assessment for security anomaly + observability legitimate-interest paths.
- **Done outputs**: [[gdpr-compliance]] + [[../30-Implementation/privacy-and-consent]] (both current binding).
...
- **Priority**: P1.
- **Q&A outcome (2026-05-18, Perplexity-backed)** — six focused sonar-pro queries (sops + age + direnv file layout, per-category rotation strategy, CI secret-injection patterns, accidental-leak response, Tier-A dependency audit, backup + recovery drills) feed [[../30-Implementation/secrets-management]]. Locked decisions:
  ...
  - **6 minimal product Q&A surfaced for Nico** (Sigstore Rekor self-hosting timing = defer post-MVP; cyber-insurance procurement = post-launch entry-tier; quarterly Tier-A audit calendar = fiscal-quarter anchored; personnel-escrow contact for paper backup = founder placeholder; annual rotation review meeting = confirmed; Tier-A initial list = confirmed). Listed in [[../30-Implementation/secrets-management]] §15.
  - **9 follow-up tasks** (FU-1..FU-9) anchored to E10 / E11 / post-MVP / founder.
- **Output files**: [[../30-Implementation/secrets-management]] (new, `status: current binding`); updates [[../30-Implementation/secrets-rotation]] (promoted from `draft` to `current binding` with explicit Cloud-Agent-credential-boundary scope; cross-references [[secrets-management]]); updates [[../00-Index/Decision-Log]], [[../00-Index/Current-State]], [[../00-Index/Implementation-Map]]; **closes F1 FU-4** (quarterly Tier-A dependency audit runbook), **closes F1 FU-6** (Sigstore Rekor outage contingency), **closes F3 FU-6** (Redis persistence drill).
- **Done outputs**: [[../30-Implementation/secrets-management]] (current binding) + [[../30-Implementation/secrets-rotation]] (current binding, narrowed scope to Cloud-Agent credential boundary).
...
- **Priority**: P1.
- **Q&A outcome (2026-05-18, Perplexity-backed)** — six focused sonar-pro queries (edge WAF graduation, per-endpoint quota catalogue, anti-griefing patterns, distributed-rate-limit pattern, bot mitigation + mCaptcha integration, escalation + admin response) feed [[../30-Implementation/rate-limiting-anti-abuse]]. Locked decisions:
  ...
  - **6 minimal product-owner Q&A** surfaced for Nico (all defaults confirmed: Phase 1 no edge WAF; Phase 3 Cloudflare gated on TIA; mCaptcha → Friendly escalation thresholds; §3 quota numerics; §4 griefing thresholds; Discord webhook as alert channel). Listed in [[../30-Implementation/rate-limiting-anti-abuse]] §12.
  - **9 follow-up tasks** (FU-1..FU-9) anchored to E10 / E11 / observability-runbook / calendar / when-triggered.
- **Output files**: [[../30-Implementation/rate-limiting-anti-abuse]] (new, `status: current binding`); updates [[../00-Index/Decision-Log]], [[../00-Index/Current-State]], [[../00-Index/Implementation-Map]]; **closes F1 Q5** (edge WAF provider choice), **closes F2 FU-5** (edge WAF + DDoS posture), **closes F3 FU-9** (per-endpoint quota numerics + edge WAF graduation).
- **Done outputs**: [[../30-Implementation/rate-limiting-anti-abuse]] (current binding).
