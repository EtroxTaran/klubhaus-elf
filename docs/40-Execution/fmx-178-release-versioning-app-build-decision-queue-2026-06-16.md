...
status: accepted
tags: [execution, decision-queue, release, versioning, app-build, dokploy, pwa, telemetry, provenance, save-compatibility, fmx-178, accepted]
created: 2026-06-16
updated: 2026-06-19
type: decision-queue
binding: true
...
> record; no open Nico decision remains for FMX-178.
...
|---|---|---|---|
| **A** | SemVer is the canonical technical app version; player-facing labels are aliases in UI/release notes. | **Yes** | See approved packet |
| B | Calendar version is the primary technical version. | No | See approved packet |
| C | Player label plus opaque build ID only. | No | See approved packet |
...
| Option | Description | Recommendation | Nico decision (2026-06-19) |
|---|---|---|---|
| **A** | Generate `release.json` with release version, player label, build ID, commit, channel, digest, content/save versions, SBOM/provenance refs and rollback predecessor. | **Yes** | See approved packet |
| B | Use runtime environment variables only. | No | See approved packet |
| C | Use `package.json#version` only. | No | See approved packet |
...
| Option | Description | Recommendation | Nico decision (2026-06-19) |
|---|---|---|---|
| **A** | Build one OCI image, promote the same digest through dev/staging/prod and roll back to a prior digest/manifest. | **Yes** | See approved packet |
| B | Rebuild separately per environment from the same git ref. | No | See approved packet |
| C | Deploy mutable tags such as `latest`, `main` or branch names as release truth. | No | See approved packet |
...
| Option | Description | Recommendation | Nico decision (2026-06-19) |
|---|---|---|---|
| **A** | Default prompt-to-refresh at safe points; no forced activation during active gameplay/session-critical flows. | **Yes** | See approved packet |
| B | Always call skip-waiting and activate immediately. | No | See approved packet |
| C | Never prompt; apply only on next full visit. | No | See approved packet |
...
| Option | Description | Recommendation | Nico decision (2026-06-19) |
|---|---|---|---|
| **A** | Track app release, API/contract version, save schema version and content pack version separately; declare compatibility per release. | **Yes** | See approved packet |
| B | Let only app major versions define compatibility. | No | See approved packet |
| C | Require a new save for every content/data change. | No | See approved packet |
...
| Option | Description | Recommendation | Nico decision (2026-06-19) |
|---|---|---|---|
| **A** | Every production digest has SBOM plus provenance/attestation references. | **Yes** | See approved packet |
| B | SBOM only. | No | See approved packet |
| C | Defer evidence until after beta. | No | See approved packet |
...
| Option | Description | Recommendation | Nico decision (2026-06-19) |
|---|---|---|---|
| **A** | Each release has a named release captain; runbook, promotion and rollback are dry-run before public beta. | **Yes** | See approved packet |
| B | Dokploy/GitHub tool owner handles release informally. | No | See approved packet |
| C | No explicit runbook until launch. | No | See approved packet |
...
No open Nico decision remains for FMX-178.
