...
rate-limiting, DDD saga vs value-object — plus the FMX-100 buffering capture).
...
`interventionPolicyVersion` / a pause-policy version).
...
PauseVoteOpened  = { watchPartyId, matchId, half, openedBy: MemberId, mode: 'vetoWindow'|'majorityVote', closesInMs: int }
PauseVoteEnacted = { watchPartyId, matchId, half, enactedBy: MemberId, budgetRemaining: int, maxDurationMs: int }
PauseResumed     = { watchPartyId, matchId, half, cause: 'autoResume'|'managerResume'|'maxDurationReached' }
PauseRequestRejected = { watchPartyId, matchId, half, requestedBy: MemberId, reasonCode: 'Cooldown'|'NoBudget'|'GlobalCapReached'|'NotAllowed'|'Vetoed'|'QuorumNotMet' }
...
PauseMatch  = { matchId: MatchId, reason: 'watchPartyVote' | 'autoDisconnect' }
ResumeMatch = { matchId: MatchId, reason: 'watchPartyVote' | 'autoDisconnect' | 'maxDuration' }
// Match → Watch-Party (already added to match.md §7 by this ADR)
MatchPaused / MatchResumed = { matchId: MatchId, reason }
```

**Persistence (draft).** `watch_party` gains distinct deliberate-pause columns —
`deliberate_pause_budget_per_half`, `deliberate_pause_global_cap_per_half`,
`deliberate_pause_cooldown_s`, `deliberate_pause_max_duration_s`, `pause_consent_mode` — and a
`watch_party_pause_event` audit child table (event log, not state). Platform-fixed ceilings
(non-configurable): `max_duration_s ≤ 60`, absolute max total paused/half (~5 min), mandatory
auto-resume, no carry-over between halves, always-on audit.

### C. §7 live-coaching matrix ownership (no overlap)
...
### Pause semantics (Design 1 — recommended default)
...
- **PV4** Per-half budget never carries over; platform ceilings (max duration ≤60s, mandatory
  auto-resume, max total paused/half) are non-configurable.
...
structure + defaults, not final magnitudes); on-device feel of the 3s veto/resume countdowns can
only be validated on a running build (tracked with the FMX-100 perf-validation follow-up).

## Out of scope / open ratification items

- All magnitudes (buffer caps; pause budget/cooldown/max-duration; quorum/veto windows) → **FMX-52**.
- Confirm **Design 1** pause semantics at ratify (this ADR defaults to it).
- Optional "tactics pause" type (longer, 1/half) — MVP vs reserved.
- Reputation-based escalation + role-based privileges (Head-Coach +1 pause / one veto-override) —
  post-MVP reserved hooks.
- The two state-machine amendments (`match.md`, `watch-party.md`) are authored `draft` in this PR;
  they flip to current only on ratify.
