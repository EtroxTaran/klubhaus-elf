...
| **SurrealDB deferred option can rot without a concrete watch trigger** | The old risk text said "stable 1.x / re-evaluate 2.x"; FMX-166 source checks show that wording is stale, and FMX-198 observed current stable patch 3.1.5 on 2026-06-19. The risk is not live data exposure — PostgreSQL remains the system of record — but stale version and maturity assumptions could mislead a future projection decision | SurrealDB stays Postgres-deferred and, where reserved, a **non-authoritative, replaceable projection** behind the existing interface ([[09-Decisions/ADR-0097-postgres-scale-envelope-and-audit-canonicalisation]], [[09-Decisions/ADR-0021-revised-tech-stack]]). Accepted FMX-166 watch: Lead Architect/Data Platform owner, quarterly light review plus event-driven review on major release, security/ops signal, backup/migration maturity or concrete FMX graph/live need; any Trial must source-check and exact-pin the current stable line at that future date, prove a rebuildable projection/disable path, and show PostgreSQL/read models cannot meet the feature target. Nico decisions D1-D5 are accepted in [[../40-Execution/fmx-166-surrealdb-deferral-watch-decision-queue-2026-06-19]] |
...
| Tempo/Mimir cut at MVP | No multi-service traces yet; pre-scale over-engineering | FMX-171 accepted trigger: Tempo only after split runtime path + one 30-minute log/metric-only localisation failure; Mimir only when 15-month Prometheus retention needs >80% TSDB disk for seven daily checks |
