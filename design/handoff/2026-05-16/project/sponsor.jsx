...
function ScreenSponsorPyramid({theme='A', scheme='light'}){
  ...
  // 5 tiers, top to bottom: prestige descending
  const TIERS = [
    { id:'jersey',   tier:1, name:'Trikotsponsor',     who:'Volta Bank',      value:1_800_000, period:'24/25 — 28/29', status:'aktiv',   accent:true,
      desc:'Frontaler Brustsponsor · 5 Saisons' },
    { id:'sleeve',   tier:2, name:'Ärmelsponsor',      who:'Lumin Telekom',   value:420_000,   period:'25/26 — 27/28', status:'aktiv',
      desc:'Beide Ärmel · drei Saisons' },
    { id:'short',    tier:3, name:'Hosensponsor',      who:'Korex Mobil',     value:180_000,   period:'26/27 — 26/27', status:'auslaufend',
      desc:'Saisonweise verlängert · läuft Sommer aus' },
    { id:'stadium',  tier:4, name:'Stadion-Naming',    who:'frei · Angebot offen', value:2_400_000, period:'27/28 — 31/32', status:'offen',
      desc:'Hafenstadt-Arena könnte „Nordis Air Arena" heißen' },
    { id:'kit',      tier:5, name:'Ausrüster',         who:'Astra Versicherung',  value:680_000,   period:'24/25 — 30/31', status:'aktiv',
      desc:'Kit-Manufacturer · 7 Jahre · langläufig' },
  ];
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100%'}}>
      ...
      <div style={{flex:1, overflowY:'auto', padding:'0 16px 20px'}}>
        ...
        {/* Gantt */}
        <div style={{fontSize:11, color:t.inkMute, fontWeight:700, letterSpacing:.6, textTransform:'uppercase', margin:'14px 2px 6px'}}>Vertragslaufzeit · 8 Jahre</div>
        <div style={{background:t.card, border:`1px solid ${t.rule}`, borderRadius:14, padding:'12px 14px'}}>
          ...
          <div style={{display:'flex', alignItems:'baseline', justifyContent:'space-between', marginTop:8, paddingTop:6, borderTop:`1px solid ${t.rule}`}}>
            <span style={{fontSize:10, color:t.accent, fontWeight:800, letterSpacing:.4}}>● HEUTE · Mai 2026</span>
            <span style={{fontSize:10, color:t.inkSoft, fontFamily:THEMES[theme].font, fontStyle:'italic'}}>Hosensponsor läuft im Sommer aus.</span>
          </div>
        </div>

        {/* Sponsor cards */}
        <div style={{fontSize:11, color:t.inkMute, fontWeight:700, letterSpacing:.6, textTransform:'uppercase', margin:'14px 2px 6px'}}>Verträge im Detail</div>
        {TIERS.map((T,i)=>(
          <SponsorRow key={T.id} T={T} theme={theme} scheme={scheme}/>
        ))}
      </div>
    </div>
  );
...
function PyramidSvg({tiers, theme, scheme}){
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H + 18}`} style={{width:'100%', display:'block'}}>
      ...
      {/* Bottom label */}
      <text x={W/2} y={H+14} textAnchor="middle" fontFamily="Inter" fontWeight="600" fontSize="9" fill={t.inkSoft} letterSpacing=".5">PROMINENZ ↑ · WERT ↑</text>
    </svg>
  );
}

function SponsorRow({T, theme, scheme}){
  const t = THEMES[theme][scheme];
    ...
    <div style={{background:t.card, border:`1px solid ${T.accent?t.accent:t.rule}`, borderRadius:12, padding:'10px 12px', marginBottom:8, display:'flex', gap:10, alignItems:'center'}}>
      ...
      <div style={{flex:1, minWidth:0}}>
        <div style={{display:'flex', alignItems:'baseline', justifyContent:'space-between'}}>
          ...
          <span style={{fontSize:10, fontWeight:800, padding:'2px 7px', borderRadius:99, background:c+'18', color:c, letterSpacing:.5, textTransform:'uppercase'}}>{T.status}</span>
        </div>
      ...
      <div style={{textAlign:'right'}}>
        <div style={{fontFamily:'JetBrains Mono', fontSize:13, fontWeight:800, color:t.ink}}>{open ? '—' : (T.value/1_000_000).toFixed(2).replace('.', ',') + ' M €'}</div>
        <div style={{fontSize:9.5, color:t.inkSoft, fontWeight:700, letterSpacing:.4, marginTop:1}}>pro Saison</div>
      </div>
