...
// =================================================================
// L8 — EXTRAHIERTE COMPOSITES (components.jsx)
// =================================================================
function LibExtracted(){
  return (
    <LibrarySection title="Extrahierte Composites"
      subtitle="Aus inline-Markup zu eigener Datei (components.jsx) extrahiert. JSDoc-Props, Story-Tags, ready für Storybook-Migration."
      cols={2}>

      <LibraryCard name="HubTile"
        file="components.jsx"
        signature={`<HubTile icon={<I.Pitch/>}
  label="Trainingsplan"
  sub="Mo–Sa · Defensive"
  flag="2 angeschlagen"
  theme="A" scheme="light"/>`}
        note="Hub-Kachel. 32px-Icon-Box, Serif-Label, Mute-Sub, scharlachrote Flag-Zeile.">
        <V label="mit Flag"><div style={{width:180}}>
          <HubTile theme="A" scheme="light"
            icon={<I.Wallet size={20} color="#1a1410"/>}
            label="Finanzen" sub="+ 632 k € / Monat"
            flag="Verbandsabgabe fällig"/>
        </div></V>
        <V label="ohne Flag"><div style={{width:180}}>
          <HubTile theme="A" scheme="light"
            icon={<I.Users size={20} color="#1a1410"/>}
            label="Transferbüro" sub="3 Anfragen offen"/>
        </div></V>
      </LibraryCard>

      <LibraryCard name="InboxCard"
        file="components.jsx"
        signature={`<InboxCard tone="board"
  from="Vorstand" title="..."
  body="..." time="vor 4 Min."
  theme="A" scheme="light"/>`}
        note="Posteingang-Karte. Tone-Glyph (§ ¶ € ◎ ♪) + 4 Aktionen.">
        <V><div style={{width:340}}>
          <InboxCard theme="A" scheme="light"
            tone="board" from="Vorstand"
            title="Saisonziel angepasst"
            body="Der Aufsichtsrat erwartet jetzt Platz 3 als Minimum."
            time="vor 4 Min."/>
        </div></V>
      </LibraryCard>

      <LibraryCard name="MatchEvent"
        file="components.jsx"
        signature={`<MatchEvent min="82'"
  kind="goal" score="2:1"
  title="TOR! Brody"
  sub="Volley aus 14 Metern."
  theme="A" scheme="light"/>`}
        note="Eine Zeile im Reportage-Feed. Goal → scharlachrot serif. Set → MiniPitch-Glyph.">
        <V><div style={{width:340, padding:'2px 14px', background:'#fbf6ea', border:`1px solid ${LIB_RULE}`, borderRadius:10}}>
          <MatchEvent theme="A" scheme="light"
            min="82'" kind="goal" score="2:1"
            title="TOR! Brody (Hafenstadt)" sub="Volley aus 14 Metern, unhaltbar."/>
          <MatchEvent theme="A" scheme="light"
            min="76'" kind="card"
            title="Gelb · Northbridge" sub="Foul an Brody."/>
          <MatchEvent theme="A" scheme="light"
            min="12'" kind="set"
            title="Eckstoß für Hafenstadt" sub="Reiter tritt — abgeklärt." last/>
        </div></V>
      </LibraryCard>

      <LibraryCard name="StatStrip"
        file="components.jsx"
        signature={`<StatStrip label="Stärke (Ø)"
  a="7,4" b="7,6"
  accentSide="b"
  hint="leichter Vorteil Hafenstadt"
  theme="A" scheme="light"/>`}
        note="Gegenüberliegende Werte mit Gewinner-Akzent. ReactNode-fähig für a/b.">
        <V><div style={{width:340, padding:'4px 14px', background:'#fbf6ea', border:`1px solid ${LIB_RULE}`, borderRadius:10}}>
          <StatStrip theme="A" scheme="light"
            label="Stärke (Ø)" a="7,4" b="7,6" accentSide="b"
            hint="leichter Vorteil Hafenstadt"/>
          <StatStrip theme="A" scheme="light"
            label="Form" mono={false}
            a={<FormStrip form="SUNSU" theme="A" scheme="light"/>}
            b={<FormStrip form="SSNSU" theme="A" scheme="light"/>} last/>
        </div></V>
      </LibraryCard>

      <LibraryCard name="AdvanceButton"
        file="components.jsx"
        signature={`<AdvanceButton
  label="Weiter zum nächsten Termin"
  daysOffset={3}
  theme="A" scheme="light"/>`}
        note="Hub-CTA. Stempel oben links zeigt Tagesoffset.">
        <V><div style={{width:340}}>
          <AdvanceButton theme="A" scheme="light"
            label="Weiter zum nächsten Termin" daysOffset={3}/>
        </div></V>
      </LibraryCard>

      <LibraryCard name="NextMatchCard"
        file="components.jsx"
        signature={`<NextMatchCard
  home="Northbridge City"
  away="Hafenstadt"
  dateLine="So 24. Mai · 15:30"
  metaLine="Aurelia Premier · 32. ST."
  theme="A" scheme="light"/>`}
        note="Hub-Tile mit Kicker, Score-Strip, optionalem Aside.">
        <V><div style={{width:340}}>
          <NextMatchCard theme="A" scheme="light"
            home="Northbridge City" away="Hafenstadt"
            dateLine="So 24. Mai · 15:30"
            metaLine="Aurelia Premier · 32. ST. · Auswärts"/>
        </div></V>
      </LibraryCard>

      <LibraryCard name="PressAnswerCard"
        file="components.jsx"
        signature={`<PressAnswerCard
  tone="höflich"
  quote="Wir respektieren..."
  predict={[
    {w:'Vorstand', d:+1},
    {w:'Fans',     d:0},
  ]}
  theme="A" scheme="light"/>`}
        note="Press-Konferenz-Antwort mit Outcome-Pills. 4 Tonalitäten.">
        <V><div style={{width:340}}>
          <PressAnswerCard theme="A" scheme="light"
            tone="höflich"
            quote="Wir respektieren Northbridge — und kommen mit eigenem Plan."
            predict={[{w:'Vorstand', d:+1}, {w:'Fans', d:0}, {w:'Spieler', d:+1}]}/>
        </div></V>
      </LibraryCard>

      <LibraryCard name="PlayerBubble"
        file="components.jsx"
        signature={`<PlayerBubble
  player={{name:'Brody',pos:'OM',form:8.6}}
  mood="energie"
  line="Lass mich tiefer fallen..."
  reactions={[
    {l:'Mehr Mut', e:'+ Risiko'},
    {l:'Bleib oben', e:'= sicher'},
  ]}
  theme="A" scheme="light"/>`}
        note="Halbzeit-Sprechblase. Portrait + MoodFace + italic-Quote.">
        <V><div style={{width:340}}>
          <PlayerBubble theme="A" scheme="light"
            player={{name:'Marek Brody', pos:'OM', form:8.6}}
            mood="energie"
      ...
      <LibraryCard name="NegotiationMessage"
        file="components.jsx"
        signature={`<NegotiationMessage
  side="them" who="Sauveterre"
  when="Mi 14:22"
  msg="Wir bleiben bei 4 Mio."
  offer={{ablöse:4_000_000}}
  theme="A" scheme="light"/>`}
        note="Chat-Bubble für Transfer-Loop. us → rechts in Ink, them → links in Card.">
        <V><div style={{width:340, display:'flex', flexDirection:'column'}}>
          <NegotiationMessage theme="A" scheme="light"
            side="them" who="Sauveterre" when="Mi 14:22"
            msg="Wir bleiben bei 4 Mio. — der Junge hat noch drei Jahre Vertrag."
            offer={{ablöse:4_000_000}}/>
          <NegotiationMessage theme="A" scheme="light"
            side="us" who="Hafenstadt" when="Mi 14:48"
            msg="4,8 Mio. + 400 k Bonus. Letztes Wort."
            offer={{ablöse:4_800_000, bonus:400_000}}/>
        </div></V>
      </LibraryCard>
...
Object.assign(window, {
  ...
  LibInputs, LibCards, LibPitch, LibStats, LibExtracted, V,
});
