...
function ScreenA11yAudit({theme='A', scheme='light'}){
  ...
  const PAIRS = [
    ...
    { fg:'#7a6f63', bg:'#f4ede0', name:'inkSoft · paper', ratio:4.5,  grade:'AA',  note: isDe ? '≥14pt' : '≥14pt' },
    { fg:'#b7301b', bg:'#f4ede0', name:'accent · paper',  ratio:5.3,  grade:'AA' },
    { fg:'#ffffff', bg:'#b7301b', name: isDe ? 'weiß · accent' : 'white · accent',   ratio:5.8,  grade:'AA' },
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100%'}}>
      ...
      <ScreenHeader theme={theme} scheme={scheme}
        kicker={isDe ? 'Audit · WCAG 2.2 AA' : 'Audit · WCAG 2.2 AA'} title={isDe ? 'Barrierefreiheit' : 'Accessibility'}
        right={
          <span style={{fontSize:11, color:t.ok, fontWeight:800, padding:'4px 10px', borderRadius:99, background:t.ok+'1f', letterSpacing:.4}}>{isDe ? '✓ AA bestanden' : '✓ AA passed'}</span>
        }/>
      ...
      <div style={{flex:1, overflowY:'auto', padding:'8px 16px 20px'}}>
        ...
        <div style={{background:t.card, border:`1px solid ${t.rule}`, borderRadius:14, padding:'12px 14px', marginBottom:10, display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:10}}>
          {[
            {k: isDe ? 'Kontrast' : 'Contrast',         v:'8/8',   sub: isDe ? 'Paare AA+' : 'pairs AA+'},
            {k: isDe ? 'Tap-Target' : 'Tap target',     v:'24/25', sub:'≥ 44×44 px'},
            {k: isDe ? 'Glyph+Farbe' : 'Glyph+colour',  v:'9/9',   sub: isDe ? 'doppelt codiert' : 'dual-coded'},
          ].map((s,i)=>(
        ...
        <div style={{fontSize:11, color:t.inkMute, fontWeight:700, letterSpacing:.6, textTransform:'uppercase', margin:'4px 2px 6px'}}>{isDe ? 'Kontrast · hell' : 'Contrast · light'}</div>
        ...
        <div style={{fontSize:11, color:t.inkMute, fontWeight:700, letterSpacing:.6, textTransform:'uppercase', margin:'4px 2px 6px'}}>{isDe ? 'Tap-Target-Karte' : 'Tap-target map'}</div>
        <div style={{background:t.card, border:`1px solid ${t.rule}`, borderRadius:12, padding:'12px 14px', marginBottom:10}}>
          ...
          <div style={{fontSize:11, color:t.inkSoft, marginTop:8, fontFamily:THEMES[theme].font, fontStyle:'italic'}}>
            {isDe ? 'Eine Stelle bleibt grenzwertig: Inbox-Mehr-Kebab. Engineering-Fix in Komponenten-Refactor.'
                  : 'One borderline item: the Inbox kebab menu. Engineering fix during the component refactor.'}
          </div>
        </div>

        <div style={{fontSize:11, color:t.inkMute, fontWeight:700, letterSpacing:.6, textTransform:'uppercase', margin:'4px 2px 6px'}}>{isDe ? 'Glyph + Farbe' : 'Glyph + colour'}</div>
        <div style={{background:t.card, border:`1px solid ${t.rule}`, borderRadius:12, padding:'10px 14px', marginBottom:10, display:'flex', flexDirection:'column', gap:6}}>
          {[
            {l: isDe ? 'Form S/U/N'         : 'Form W/D/L',                 g: isDe ? 'S U N' : 'W D L', note: isDe ? 'Buchstabe + Farbe' : 'letter + colour'},
            {l: isDe ? 'Vorhersage ▲▼=' : 'Prediction ▲▼=',          g:'▲ = ▼',                  note: isDe ? 'Glyphe + Farbe'    : 'glyph + colour'},
            {l: isDe ? 'Vertrag läuft aus' : 'Contract expiring',          g:'●',                      note: isDe ? 'Punkt + Datum rot' : 'dot + red date'},
            {l: isDe ? 'Dach offen/teil/voll' : 'Roof open/partial/full',   g:'⎴ ⎴ ⎴',               note: isDe ? 'Stroke + Dash + Farbe' : 'stroke + dash + colour'},
          ].map((r,i)=>(
        ...
        <div style={{fontSize:11, color:t.inkMute, fontWeight:700, letterSpacing:.6, textTransform:'uppercase', margin:'4px 2px 6px'}}>{isDe ? 'Offene Punkte · 6 Items' : 'Open items · 6'}</div>
        <div style={{background:t.card, border:`1px solid ${t.rule}`, borderRadius:12, padding:'10px 14px'}}>
          {(isDe ? [
            ...
            'aria-hidden on decorative <Crest/> when the club name is also rendered.',
            'role="status" on LiveXgStrip for live-region announcements.',
            'Use <dl>/<dt>/<dd> instead of <div> for stat tiles (Kpi · Stat · Sum).',
            'Focus trap on Halftime sheet, Settings sheet, Press conference.',
            'prefers-reduced-motion: cinematic.* should snap to final frame.',
            'Bump the Inbox kebab to 44×44 px (currently 36).',
          ]).map((todo,i,arr)=>(
            <div key={i} style={{display:'flex', alignItems:'flex-start', gap:10, padding:'7px 0', borderBottom: i<arr.length-1?`1px solid ${t.rule}`:'none'}}>
