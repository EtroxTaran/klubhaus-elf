...
// ---------- SHARED ATOMS (themed) ----------
//
// CSS Variables · Migration Foundation
// ────────────────────────────────────────────────────────────────
// Two layers coexist:
//   1) Legacy: components read `const t = THEMES[theme][scheme]` and use
//      `t.accent` inline. Still works.
//   2) New:    components read `var(--accent)` directly in style attributes
//      or Tailwind classes. Re-renders are no longer needed when theme tokens
//      change — the CSS layer updates in place.
//
// Both layers are emitted by <ThemeCss/>. Phone-screens get the variables
// scoped to `.phone-screen`; arbitrary container scopes can use the
// `[data-theme][data-scheme]` selector pattern, which `<ThemeRoot/>` injects.
// ────────────────────────────────────────────────────────────────
function ThemeCss({ theme, scheme }) {
  const t = THEMES[theme][scheme];
  // Emit ALL tokens as CSS custom properties, scoped to the nearest
  // .phone-screen AND to any [data-theme=...][data-scheme=...] container.
  const css = Object.entries(t).map(([k,v])=>`--${k}:${v};`).join('');
  return <style>{`
    .phone-screen{${css}}
    [data-theme="${theme}"][data-scheme="${scheme}"]{${css}}
  `}</style>;
}

// <ThemeRoot> — opt-in wrapper for components that want to use var(--x)
// without sitting inside .phone-screen. Sets the data-attrs that ThemeCss's
// rules target.
//
//   <ThemeRoot theme="A" scheme="light">
//     <MyComponent/>   {/* can use color: var(--accent) freely */}
//   </ThemeRoot>
function ThemeRoot({ theme='A', scheme='light', as:Tag='div', children, ...rest }){
  return (
    <Tag data-theme={theme} data-scheme={scheme} {...rest}>
...
Object.assign(window, {
  ...
  I, PhoneFrame, ThemeCss, ThemeRoot, useTokens,
