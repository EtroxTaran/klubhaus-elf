    ...
    <div style={{display:'flex',flexDirection:'column',height:'100%', position:'relative'}}>
      ...
      <header style={{background:t.card, borderBottom:`1px solid ${t.rule}`, padding:'8px 14px 10px'}}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
          <button aria-label={tr('common.back')} style={{width:36,height:36,borderRadius:10,background:t.bgInk,border:`1px solid ${t.rule}`,display:'grid',placeItems:'center'}}><I.ChevronLeft color={t.ink} size={18}/></button>
          <div style={{fontSize:10, color:t.inkMute, fontWeight:700, letterSpacing:.4, textAlign:'center', textTransform:'uppercase'}}>{isDe ? 'Aurelia Premier · 32. ST.' : 'Aurelia Premier · MD 32'}</div>
          <button aria-label={tr('settings.title')} style={{width:36,height:36,borderRadius:10,background:t.bgInk,border:`1px solid ${t.rule}`,display:'grid',placeItems:'center'}}><I.Settings color={t.ink} size={16}/></button>
        </div>
        ...
        <div style={{display:'flex', alignItems:'center', justifyContent:'center', gap:10, marginTop:6}}>
          ...
          <span style={{fontSize:11, color:t.inkMute}}>{isDe ? '27.412 Zuschauer' : '27,412 attendance'}</span>
        </div>
        {/* xG live strip — T2.3 */}
        ...
        <div style={{display:'flex', marginTop:8, background:t.bgInk, borderRadius:10, padding:3}}>
          {[{l:tr('match.tab.feed'),a:true},{l:tr('match.tab.ticker')},{l:tr('match.tab.lineup')}].map(c=>(
      ...
      {/* Event feed — uses extracted MatchEvent composite */}
      ...
      <div style={{position:'absolute',left:0,right:0,bottom:0,padding:'10px 14px 22px',background:`linear-gradient(to top, ${t.bg}, ${t.bg}f0 60%, transparent)`,display:'flex',gap:8}}>
        <button style={{flex:1, height:48, borderRadius:14, background:t.card, border:`1px solid ${t.rule}`, color:t.ink, fontWeight:700, fontFamily:'inherit', display:'flex',alignItems:'center',justifyContent:'center',gap:6, fontSize:13}}><I.Whistle size={16} color={t.ink}/> {tr('match.pause')}</button>
        <button style={{flex:2, height:48, borderRadius:14, background:t.ink, border:'none', color:t.bg, fontWeight:800, fontFamily:'inherit', display:'flex',alignItems:'center',justifyContent:'center',gap:6, fontSize:14}}>{tr('match.tempo')} &nbsp;❯❯ &nbsp; <span style={{opacity:.6, fontWeight:600, fontSize:11}}>({tr('match.tempo.x2')})</span></button>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100%', position:'relative', background:t.bg}}>
      ...
      <div style={{padding:'16px', opacity:.45, filter:'blur(.3px)'}}>
        <SerifH theme={theme} style={{fontSize:12, fontWeight:700, color:t.inkMute, textTransform:'uppercase', letterSpacing:.6}}>{tr('match.halftime')} · 45'</SerifH>
        <SerifH theme={theme} style={{display:'block', fontSize:30, fontWeight:800, color:t.ink}}>Northbridge 0:1 Hafenstadt</SerifH>
        <div style={{fontSize:12, color:t.inkMute, marginTop:4}}>{isDe ? 'Verdiente Führung dank Wieser (34’). Brody hängt durch.' : 'Deserved lead via Wieser (34’). Brody is dragging his heels.'}</div>
      </div>
      ...
      <div style={{
        ...
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'baseline'}}>
          <div>
            ...
            <SerifH theme={theme} style={{display:'block', fontSize:22, fontWeight:700, color:t.ink, lineHeight:1.1}}>{isDe ? 'Was passen wir an?' : 'What do we change?'}</SerifH>
          </div>
          <span style={{fontSize:11, color:t.accent, fontWeight:700}}>{isDe ? '3 Min. Pause' : '3 min break'}</span>
        </div>

        {/* Formation */}
        <div style={{marginTop:14}}>
          <div style={{fontSize:11, color:t.inkMute, fontWeight:700, letterSpacing:.5, textTransform:'uppercase', marginBottom:6}}>{tr('halftime.formation')}</div>
          <div style={{display:'flex', gap:6, overflowX:'auto', padding:'2px 0 4px'}}>
            {['4-3-3','4-4-2','4-2-3-1','3-5-2','5-3-2'].map((f,i)=>(
              ...
              }}>{f}{i===0 && <div style={{fontSize:9, fontWeight:600, opacity:.7, marginTop:1, fontFamily:'Inter'}}>{isDe ? 'aktuell' : 'current'}</div>}</div>
            ))}
          </div>
        ...
        <div style={{marginTop:8}}>
          <div style={{fontSize:11, color:t.inkMute, fontWeight:700, letterSpacing:.5, textTransform:'uppercase', marginBottom:6}}>{tr('halftime.mentality')}</div>
          <div style={{display:'flex', gap:6}}>
            {[
              {l:tr('halftime.mentality.safe')},
              {l:tr('halftime.mentality.balanced'), a:true},
              {l:tr('halftime.mentality.press')}
            ].map((m,i)=>(
              <button key={i} style={{
        ...
        <div style={{marginTop:10, padding:'10px 12px', background:t.bgInk, borderRadius:12, border:`1px dashed ${t.rule}`}}>
          <div style={{display:'flex', alignItems:'center', justifyContent:'space-between'}}>
            <div style={{fontSize:11, color:t.inkMute, fontWeight:700, letterSpacing:.5, textTransform:'uppercase'}}>{tr('halftime.suggest_sub')}</div>
            <span style={{fontSize:10, color:t.accent, fontWeight:700}}>{isDe ? 'Co-Trainer empfiehlt' : 'Asst. coach suggests'}</span>
          </div>
          <div style={{display:'flex', alignItems:'center', gap:10, marginTop:8, fontSize:13}}>
            ...
            <span style={{flex:1}}/>
            <span style={{fontSize:11, color:t.inkMute, textAlign:'right'}}>{isDe ? 'frischer' : 'fresher'}<br/>{isDe ? '+0,4 Form' : '+0.4 form'}</span>
          </div>
        </div>
        ...
        {/* Bottom buttons */}
        <button style={{width:'100%', height:32, marginTop:10, marginBottom:8, background:'transparent', border:'none', color:t.inkMute, fontWeight:600, fontSize:12, fontFamily:'inherit', display:'flex', alignItems:'center', justifyContent:'center', gap:4}}>{tr('halftime.more_tactics')} <I.ChevronDown size={14} color={t.inkMute}/></button>
        <div style={{display:'flex', gap:8}}>
          <button style={{flex:1, height:52, borderRadius:14, background:t.bg, border:`1px solid ${t.rule}`, color:t.ink, fontWeight:700, fontSize:14, fontFamily:'inherit'}}>{isDe ? 'Wie bisher' : 'Keep as is'}</button>
          <button style={{flex:2, height:52, borderRadius:14, background:t.accent, color:'#fff', border:'none', fontWeight:800, fontSize:15, fontFamily:'inherit', boxShadow:`0 6px 14px -4px ${t.accent}80`}}>{isDe ? 'Übernehmen & weiter' : 'Apply & continue'}</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100%'}}>
      ...
      <ScreenHeader theme={theme} scheme={scheme}
        kicker={isDe ? 'Mai 2026' : 'May 2026'} title={tr('finance.title')}
      ...
      <div style={{padding:'0 16px 12px'}}>
        <div style={{background:t.card, border:`1px solid ${t.rule}`, borderRadius:14, padding:'12px 14px'}}>
          <div style={{display:'flex', justifyContent:'space-between'}}>
            <div>
              <div style={{fontSize:10, color:t.inkMute, fontWeight:700, letterSpacing:.6, textTransform:'uppercase'}}>{isDe ? 'Kontostand' : 'Cash on hand'}</div>
            ...
            <div style={{textAlign:'right'}}>
              <div style={{fontSize:10, color:t.inkMute, fontWeight:700, letterSpacing:.6, textTransform:'uppercase'}}>{tr('finance.balance')}</div>
              <div style={{display:'inline-flex', alignItems:'center', gap:4, fontSize:18, fontWeight:800, color:t.ok, fontFamily:'JetBrains Mono', marginTop:2}}><I.TrendUp size={16} color={t.ok}/>{isDe ? '+ 632.000 €' : '+ €632k'}</div>
            </div>
          </div>
          <div style={{display:'flex', gap:6, marginTop:10}}>
            {[{l:tr('finance.tab.operations'), a:true},{l:tr('finance.tab.investment')},{l:tr('finance.tab.history')}].map(c=>(
      ...
      <div style={{flex:1, overflowY:'auto', padding:'0 16px 20px'}}>
        <div style={{fontSize:11, color:t.inkMute, fontWeight:700, letterSpacing:.6, textTransform:'uppercase', margin:'6px 2px'}}>{isDe ? 'Einnahmen' : 'Revenue'}</div>
        ...
        <div style={{fontSize:11, color:t.inkMute, fontWeight:700, letterSpacing:.6, textTransform:'uppercase', margin:'14px 2px 6px'}}>{isDe ? 'Ausgaben' : 'Expenses'}</div>
        <div style={{background:t.card, border:`1px solid ${t.rule}`, borderRadius:12, padding:'2px 14px'}}>
          {f.expenses.map((x,i)=>(
        ...
        <div style={{fontSize:11, color:t.inkMute, fontWeight:700, letterSpacing:.6, textTransform:'uppercase', margin:'14px 2px 6px'}}>{isDe ? 'Investitionsbudget' : 'Investment budget'}</div>
        ...
        {/* Levers */}
        <div style={{fontSize:11, color:t.inkMute, fontWeight:700, letterSpacing:.6, textTransform:'uppercase', margin:'14px 2px 6px'}}>{isDe ? 'Hebel' : 'Levers'}</div>
        <div style={{background:t.card, border:`1px solid ${t.rule}`, borderRadius:12, padding:'14px'}}>
          <Slider theme={theme} scheme={scheme} label={isDe ? 'Ticketpreis · Stehplatz' : 'Ticket price · standing'} value={62} unit="%" tooltip={isDe ? '14 € / Karte' : '€14 / ticket'} leftL="-30%" rightL="+30%"/>
          <Slider theme={theme} scheme={scheme} label={isDe ? 'Trikotsponsoring' : 'Shirt sponsor'} value={48} unit="%" tooltip={isDe ? '1,8 Mio. € / Saison' : '€1.8M / season'} leftL={isDe ? 'bescheiden' : 'modest'} rightL={isDe ? 'aggressiv' : 'aggressive'}/>
          <Slider theme={theme} scheme={scheme} label={isDe ? 'Nachwuchs-Etat' : 'Youth budget'} value={71} unit="%" tooltip={isDe ? '180.000 € / Monat' : '€180k / month'} leftL={isDe ? 'kühl' : 'cool'} rightL={isDe ? 'großzügig' : 'generous'} last/>
        </div>
      </div>
    </div>
  );
...
function ScreenStadium({theme, scheme}){
  ...
  const Row = ({glyph, name, status, detail, roi, cost, soft, danger, action}) => (
    <div style={{background:t.card, border:`1px solid ${t.rule}`, borderRadius:12, padding:'10px 12px', display:'flex', gap:10, alignItems:'center', marginBottom:8}}>
      ...
      <div style={{flex:1, minWidth:0}}>
        ...
        <div style={{display:'flex', gap:10, marginTop:5, alignItems:'baseline', flexWrap:'wrap'}}>
          ...
          {cost && <span style={{fontSize:10.5, color:t.inkSoft, fontFamily:'JetBrains Mono'}}>· {isDe ? 'Kosten' : 'Cost'} {cost}</span>}
        </div>
      </div>
      <button aria-label={action || (soft ? (isDe?'bauen':'build') : (isDe?'ausbauen':'upgrade'))} style={{
  ...
  const StandCard = ({s}) => {
    ...
    const roofLabel = {
      full: isDe ? 'Dach komplett' : 'Roof full',
      partial: isDe ? 'Dach teilweise' : 'Roof partial',
      open: isDe ? 'Ohne Dach' : 'No roof',
    }[s.roof];
    return (
          ...
          <div style={{display:'flex', alignItems:'center', gap:10}}>
            ...
            <div>
              <SerifH theme={theme} style={{fontSize:15, fontWeight:700, color:t.ink, lineHeight:1.05}}>{s.name}</SerifH>
                ...
                <span style={{fontFamily:'JetBrains Mono', fontWeight:700, color:t.ink}}>{new Intl.NumberFormat(isDe?'de-DE':'en-GB').format(s.cap)}</span>
                <span>{isDe ? 'Plätze' : 'seats'} · {s.rows} {isDe ? 'Reihen' : 'rows'} · {s.blocks} {isDe ? 'Blöcke' : 'blocks'}</span>
              </div>
            </div>
          </div>
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100%'}}>
      ...
      <ScreenHeader theme={theme} scheme={scheme}
        ...
        title={isDe ? 'Stadionausbau' : 'Stadium upgrade'}
        right={
          <div style={{textAlign:'right'}}>
            <div style={{fontSize:10, color:t.inkMute, fontWeight:700, letterSpacing:.6, textTransform:'uppercase'}}>{tr('stadium.capacity')}</div>
            <SerifH theme={theme} style={{fontSize:18, fontWeight:800, color:t.ink, fontFamily:'JetBrains Mono'}}>{isDe ? '27.412' : '27,412'}</SerifH>
          </div>
        }/>
      ...
      <div style={{padding:'4px 12px 0'}}>
        <div style={{display:'flex', gap:4, padding:3, background:t.bgInk, borderRadius:10}}>
          {[
            {id:'anlage',   l:isDe ? 'Anlage' : 'Layout'},
            {id:'tribuene', l:isDe ? 'Tribünen' : 'Stands'},
            {id:'rasen',    l:isDe ? 'Rasen & Licht' : 'Pitch & lights'},
            {id:'gastro',   l:isDe ? 'Gastro' : 'F&B'},
            {id:'typen',    l:isDe ? 'Stadiontyp' : 'Stadium type'},
          ].map(o=>(
      ...
      <div style={{flex:1, overflowY:'auto', padding:'10px 12px 20px'}}>
        ...
        {tab === 'anlage' && (<>
          ...
          <SectionLabel k={isDe ? 'Anbauten' : 'Add-ons'} c={isDe ? 'Budget · 8,5 Mio. €' : 'Budget · €8.5M'}/>
        ...
        {tab === 'tribuene' && (<>
          <SectionLabel k={isDe ? 'Vier Tribünen · 27.412 Plätze gesamt' : 'Four stands · 27,412 seats total'}/>
          ...
          <div style={{display:'flex', gap:6, marginBottom:8, padding:'8px 10px', background:t.bgInk, borderRadius:10, alignItems:'center'}}>
            <div style={{flex:1, fontSize:11, color:t.inkMute, fontWeight:700}}>{isDe ? 'Überdachung' : 'Roofing'}</div>
            {[
        ...
        {tab === 'rasen' && (<>
          <SectionLabel k={isDe ? 'Rasen, Drainage & Beleuchtung' : 'Pitch, drainage & lighting'}/>
        ...
        {tab === 'gastro' && (<>
          <SectionLabel k={isDe ? 'Gastronomie · Spieltagserlös' : 'Food & beverage · matchday revenue'} c={isDe ? 'Ø 27.000 € / Heimspiel' : 'Ø €27k / home match'}/>
          {GASTRO.map((g,i)=>{
        ...
        {tab === 'typen' && (<>
          <SectionLabel k={isDe ? 'Stadiontypen · Ausbaupfad' : 'Stadium types · upgrade path'}/>
...
function ScreenOnboardingCountry({theme, scheme}){
  ...
  const isDe = getLocale() === 'de';
  const COUNTRIES = [
    {n: isDe?'Deutschland':'Germany', flag:'🇩🇪', league:'Aurelia Premier', tier: (isDe?'1. Liga · 18 Klubs':'Tier 1 · 18 clubs'), sel:true},
    {n: isDe?'Italien':'Italy',       flag:'🇮🇹', league:'Liga Norvania',   tier: (isDe?'1. Liga · 20 Klubs':'Tier 1 · 20 clubs')},
    {n: isDe?'Frankreich':'France',   flag:'🇫🇷', league:'Liga Norvania',   tier: (isDe?'1. Liga · 20 Klubs':'Tier 1 · 20 clubs')},
    {n:'Japan',                        flag:'🇯🇵', league:'Liga Norvania',   tier: (isDe?'1. Liga · 18 Klubs':'Tier 1 · 18 clubs')},
    {n:'Portugal',                     flag:'🇵🇹', league:'Liga Norvania',   tier: (isDe?'1. Liga · 18 Klubs':'Tier 1 · 18 clubs')},
  ];
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100%', position:'relative'}}>
      <ThemeCss theme={theme} scheme={scheme}/>
      <header style={{padding:'4px 16px 8px'}}>
        ...
        <div style={{fontSize:10, fontWeight:700, color:t.inkMute, letterSpacing:.6, marginTop:10, textTransform:'uppercase'}}>{isDe ? 'Schritt 1 von 3' : 'Step 1 of 3'}</div>
        <SerifH theme={theme} style={{display:'block', fontSize:28, fontWeight:700, color:t.ink, lineHeight:1.05, marginTop:2}}>{isDe ? <>Wo soll Ihr Fußball-<br/>kosmos beginnen?</> : <>Where shall your football<br/>universe begin?</>}</SerifH>
        <div style={{fontSize:13, color:t.inkMute, marginTop:6, fontFamily:THEMES[theme].font, fontStyle:'italic'}}>{isDe ? 'Echtes Land, frei erfundene Liga. Funktioniert ohne Netz.' : 'Real country, invented league. Works offline.'}</div>
      </header>
      ...
      <div style={{position:'absolute', left:0, right:0, bottom:0, padding:'10px 16px 22px', background:t.bg}}>
        <button style={{width:'100%', height:54, borderRadius:14, background:t.accent, color:'#fff', border:'none', fontWeight:800, fontSize:15, fontFamily:'inherit', display:'flex', alignItems:'center', justifyContent:'center', gap:8}}>{tr('common.next')} <I.ArrowRight size={18} color="#fff"/></button>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100%', position:'relative'}}>
      ...
      <header style={{padding:'4px 16px 8px'}}>
        ...
        <div style={{fontSize:10, fontWeight:700, color:t.inkMute, letterSpacing:.6, marginTop:10, textTransform:'uppercase'}}>{isDe ? 'Schritt 2 von 3' : 'Step 2 of 3'}</div>
        <SerifH theme={theme} style={{display:'block', fontSize:24, fontWeight:700, color:t.ink, lineHeight:1.05, marginTop:2}}>{isDe ? <>Wer braucht Sie<br/>am dringendsten?</> : <>Who needs you<br/>most urgently?</>}</SerifH>
      </header>
      ...
      <div style={{position:'absolute', left:0, right:0, bottom:0, padding:'10px 16px 22px', background:t.bg, display:'flex', gap:8}}>
        <button style={{flex:1, height:50, borderRadius:14, background:'transparent', color:t.ink, border:`1px solid ${t.rule}`, fontWeight:700, fontSize:14, fontFamily:'inherit'}}>{tr('onboarding.dice')}</button>
        <button style={{flex:2, height:50, borderRadius:14, background:t.accent, color:'#fff', border:'none', fontWeight:800, fontSize:15, fontFamily:'inherit', display:'flex', alignItems:'center', justifyContent:'center', gap:8}}>{isDe ? 'FC Hafenstadt nehmen' : 'Take FC Hafenstadt'} <I.ArrowRight size={18} color="#fff"/></button>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100%', position:'relative'}}>
      ...
      <header style={{padding:'4px 16px 8px'}}>
        ...
        <div style={{fontSize:10, fontWeight:700, color:t.inkMute, letterSpacing:.6, marginTop:10, textTransform:'uppercase'}}>{isDe ? 'Schritt 3 von 3' : 'Step 3 of 3'}</div>
        <SerifH theme={theme} style={{display:'block', fontSize:24, fontWeight:700, color:t.ink, lineHeight:1.05, marginTop:2}}>{isDe ? 'Und Sie sind…?' : 'And you are…?'}</SerifH>
        <div style={{fontSize:12, color:t.inkMute, marginTop:4, fontFamily:THEMES[theme].font, fontStyle:'italic'}}>{isDe ? 'Ein Initial-Chip reicht für die erste Saison.' : 'An initials chip is enough for season one.'}</div>
      </header>
      ...
      <div style={{padding:'18px 16px 0'}}>
        <div style={{fontSize:11, color:t.inkMute, fontWeight:700, letterSpacing:.6, textTransform:'uppercase', marginBottom:6}}>{isDe ? 'Name' : 'Name'}</div>
        <div style={{background:t.card, border:`1px solid ${t.rule}`, borderRadius:12, padding:'12px 14px', display:'flex', alignItems:'center', gap:8}}>
          ...
          <span style={{flex:1}}/>
          <button style={{fontSize:11, color:t.accent, fontWeight:700, background:'transparent', border:'none'}}>{isDe ? 'ändern' : 'edit'}</button>
        </div>
        <div style={{fontSize:11, color:t.inkMute, fontWeight:700, letterSpacing:.6, textTransform:'uppercase', margin:'14px 0 6px'}}>{isDe ? 'Spitzname (optional)' : 'Nickname (optional)'}</div>
        <div style={{background:t.card, border:`1px solid ${t.rule}`, borderRadius:12, padding:'12px 14px', color:t.inkSoft, fontFamily:THEMES[theme].font, fontStyle:'italic'}}>{isDe ? '„Die Hand vom Hafen“' : '“The Hand of the Harbour”'}</div>
      </div>

      <div style={{flex:1}}/>
      <div style={{position:'absolute', left:0, right:0, bottom:0, padding:'10px 16px 22px', background:t.bg}}>
        <button style={{width:'100%', height:56, borderRadius:14, background:t.ink, color:t.bg, border:'none', fontWeight:800, fontSize:16, fontFamily:'inherit', display:'flex', alignItems:'center', justifyContent:'center', gap:8}}>{tr('onboarding.start')} <I.ArrowRight size={20} color={t.bg}/></button>
      </div>
    </div>
  );
}
    ...
    <div style={{display:'flex',flexDirection:'column',height:'100%'}}>
      ...
      <header style={{padding:'4px 16px 12px'}}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-end'}}>
          <div>
            <div style={{fontSize:11,color:t.inkMute, fontWeight:600, letterSpacing:.4, textTransform:'uppercase'}}>{isDe ? 'Karriereverwaltung' : 'Saved careers'}</div>
            <SerifH theme={theme} style={{display:'block', fontSize:26, fontWeight:700, color:t.ink, lineHeight:1}}>{isDe ? 'Meine Karrieren' : 'My careers'}</SerifH>
          </div>
        ...
        <div style={{display:'flex', alignItems:'center', gap:6, marginTop:10, padding:'8px 10px', background:t.bgInk, borderRadius:10}}>
          ...
          <span style={{fontSize:11, color:t.inkMute, fontWeight:600}}>{isDe ? 'Offline gespeichert · 77 MB von 250 MB belegt' : 'Offline saves · 77 MB of 250 MB used'}</span>
        </div>
      </header>
      ...
      <div style={{flex:1, overflowY:'auto', padding:'0 16px 16px'}}>
        {SAVE_SLOTS.map((s,i)=>{
          if (s.empty) return (
            <button key={i} style={{
              ...
              <span style={{fontSize:14, fontWeight:700}}>{isDe ? 'Neue Karriere starten' : 'Start a new career'}</span>
              <span style={{fontSize:11}}>{isDe ? `Slot ${s.i} · frei` : `Slot ${s.i} · empty`}</span>
            </button>
          );
          return (
            <div key={i} style={{background:t.card, border:`1px solid ${t.rule}`, borderRadius:14, padding:'14px', marginBottom:10}}>
                ...
                <div style={{flex:1, minWidth:0}}>
                  ...
                  <div style={{fontSize:11, color:t.inkMute, marginTop:2}}>
                    <span style={{display:'inline-flex', alignItems:'center', gap:4}}>
                      ...
                      {s.club} · {isDe ? `Saison ${s.season}` : `Season ${s.season}`}
                    </span>
                  </div>
                </div>
              ...
              <div style={{marginTop:10}}>
                <div style={{display:'flex', justifyContent:'space-between', fontSize:10, color:t.inkMute, marginBottom:3, fontFamily:'JetBrains Mono'}}>
                  <span>{s.size} MB</span>
                  <span>{Math.round(s.ratio*100)}{isDe ? '% des Quotas' : '% of quota'}</span>
                </div>
              ...
              <div style={{display:'flex', gap:6, marginTop:12}}>
                <button style={{flex:1, height:42, borderRadius:10, background:t.ink, color:t.bg, border:'none', fontWeight:800, fontSize:13, fontFamily:'inherit'}}>{isDe ? 'Fortsetzen' : 'Continue'}</button>
                <button aria-label={isDe ? 'Exportieren' : 'Export'} style={{width:42, height:42, borderRadius:10, background:t.bg, border:`1px solid ${t.rule}`, display:'grid', placeItems:'center'}}><I.Download size={16} color={t.ink}/></button>
                <button aria-label={isDe ? 'Importieren' : 'Import'} style={{width:42, height:42, borderRadius:10, background:t.bg, border:`1px solid ${t.rule}`, display:'grid', placeItems:'center'}}><I.Upload size={16} color={t.ink}/></button>
                <button aria-label={tr('common.delete')} style={{width:42, height:42, borderRadius:10, background:t.bg, border:`1px solid ${t.rule}`, display:'grid', placeItems:'center'}}><I.Trash size={16} color={t.danger}/></button>
              </div>
            </div>
          );
        })}
        ...
        <div style={{background:t.accentSoft, border:`1px solid ${t.accent}`, borderRadius:14, padding:'12px 14px', marginTop:6, display:'flex', gap:10, alignItems:'flex-start'}}>
          ...
          <div style={{flex:1}}>
            <SerifH theme={theme} style={{display:'block', fontSize:14, fontWeight:800, color:t.accent}}>{isDe ? 'Auf Startbildschirm hinzufügen' : 'Add to home screen'}</SerifH>
            <div style={{fontSize:11.5, color:t.ink, marginTop:2, lineHeight:1.35}}>{isDe ? <>So spielen Sie offline wie eine echte App. <span style={{color:t.inkMute}}>Im Safari-Menü tippen → <I.ShareIos size={11} color={t.inkMute}/> „Zum Home-Bildschirm“.</span></> : <>Play offline like a real app. <span style={{color:t.inkMute}}>In Safari, tap → <I.ShareIos size={11} color={t.inkMute}/> “Add to Home Screen”.</span></>}</div>
          </div>
        </div>
      </div>
    </div>
